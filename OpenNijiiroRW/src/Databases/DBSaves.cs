...
internal class DBSaves
{
	...
	public static bool RegisterPlay(int player, int clearStatus, int scoreRank)
	{
		...
		SaveFile.Data saveData = OpenNijiiroRW.SaveFileInstances[OpenNijiiroRW.GetActualPlayer(player)].data;
		BestPlayRecords.CBestPlayRecord currentPlay = new BestPlayRecords.CBestPlayRecord();
		var choosenSong = OpenNijiiroRW.stageSongSelect.rChoosenSong;
		var choosenDifficulty = OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[player];
		var chartScore = OpenNijiiroRW.stageGameScreen.CChartScore[player];
...

		{
			...
			currentPlay.HighScoreMaxCombo = OpenNijiiroRW.stageGameScreen.actCombo.nCurrentCombo.最高値[player];
