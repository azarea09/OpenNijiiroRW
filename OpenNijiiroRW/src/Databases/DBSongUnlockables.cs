...
using static OpenNijiiroRW.DBSongUnlockables;
	...
	public DBSongUnlockables() {
		...
		using (var connection = new SqliteConnection(@$"Data Source={_fn}")) {
			...
			while (reader.Read()) {
				...
				su.unlockConditions = OpenNijiiroRW.UnlockConditionFactory.GenerateUnlockObjectFromJsonRaw(_raw);
				...
				if (_fulfilled) {
					...
					mq.tAddModal(
						new Modal(
							...
							_node,
							OpenNijiiroRW.stageSongSelect.actPreimageパネル.tGenerateAndGetPreimage
						),
						_player);

					DBSaves.RegisterStringUnlockedAsset(OpenNijiiroRW.SaveFileInstances[player].data.SaveId, "unlocked_songs", _npvKey);
