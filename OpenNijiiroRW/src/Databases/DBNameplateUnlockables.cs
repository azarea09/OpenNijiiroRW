...
using static OpenNijiiroRW.DBNameplateUnlockables;
	...
	public DBNameplateUnlockables()
	{
		...
		using (var connection = new SqliteConnection(@$"Data Source={_fn}"))
		{
			...
			while (reader.Read())
			{
				...
				nu.unlockConditions = OpenNijiiroRW.UnlockConditionFactory.GenerateUnlockObjectFromJsonRaw(_raw);
				...
				if (_fulfilled)
				{
					...
					mq.tAddModal(
						new Modal(
							...
							item,
							OpenNijiiroRW.NamePlate.lcNamePlate
						),
						_player);

					DBSaves.RegisterUnlockedNameplate(OpenNijiiroRW.SaveFileInstances[player].data.SaveId, _npvKey);
				}
			}
		}
