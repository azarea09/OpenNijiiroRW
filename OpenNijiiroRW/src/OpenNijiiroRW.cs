...
internal class OpenNijiiroRW : Game
{
	...
	protected override void Draw()
	{
...
#endif
		{
			...
			if (rCurrentStage != null)
			{
					...
					case CStage.EStage.SongLoading:
						if (EnumSongs != null)
						{
								...
								!EnumSongs.IsSongListEnumStarted)
							{
								...
								OpenNijiiroRW.stageSongSelect.bIsEnumeratingSongs = true;
							...
							if (EnumSongs.IsSongListEnumerated)
							{
								...
								OpenNijiiroRW.stageSongSelect.bIsEnumeratingSongs = false;

								bool bRemakeSongTitleBar = (rCurrentStage.eStageID == CStage.EStage.SongSelect) ? true : false;
								OpenNijiiroRW.stageSongSelect.Refresh(EnumSongs.Songs管理, bRemakeSongTitleBar);
					...
					case CStage.EStage.Title:
						...
						switch (this.nDrawLoopReturnValue)
						{
							case (int)CStageTitle.EReturnValue.GAMESTART:
								...
								OpenNijiiroRW.latestSongSelect = stageSongSelect;
								//-----------------------------
								#endregion
								break;
							...
							case (int)CStageTitle.EReturnValue.DANGAMESTART:
								...
								OpenNijiiroRW.latestSongSelect = stageDanSongSelect;
								//-----------------------------
								#endregion
								break;
							...
							case (int)CStageTitle.EReturnValue.TAIKOTOWERSSTART:
								...
								OpenNijiiroRW.latestSongSelect = stageTowerSelect;
								//-----------------------------
								#endregion
								break;
					...
					case CStage.EStage.CutScene:
						...
						switch (this.nDrawLoopReturnValue)
						{
							...
							case (int)CStageCutScene.EReturnValue.OutroFinished:
								...
								this.ReturnToSongSelection(OpenNijiiroRW.stageResults);
					...
					case CStage.EStage.SongLoading:
						...
						if (this.nDrawLoopReturnValue != 0)
						{
							...
							if (this.nDrawLoopReturnValue == (int)ESongLoadingScreenReturnValue.LoadCanceled)
							{
								...
								MountStage(OpenNijiiroRW.latestSongSelect);

								rPreviousStage = rCurrentStage;

								// Seek latest registered song select screen
								rCurrentStage = OpenNijiiroRW.latestSongSelect;
				...
				// テスト
				OpenNijiiroRW.actTextConsole.Print(10, 10, CTextConsole.EFontType.WhiteSlim, $"FPS: {(OpenNijiiroRW.FPS != null ? OpenNijiiroRW.FPS.NowFPS : "???")}");
	...
	private void tStartupProcess()
	{
		...
		if (ConfigIni.bOutputLogs)
		{
			...
			catch (System.UnauthorizedAccessException)            // #24481 2011.2.20 yyagi
			{
				...
				string[] mes_writeErr = {
					"OpenNijiiroRW.logへの書き込みができませんでした。書き込みできるようにしてから、再度起動してください。",
					"Failed to write OpenNijiiroRW.log. Please set your device to READ/WRITE and try again."
				};
		...
		Trace.WriteLine("");
		Trace.WriteLine("Welcome to OpenNijiiroRW! Starting log...");
...
#endif
		{
			Skin = new CSkin(OpenNijiiroRW.ConfigIni.strSystemSkinSubfolderFullName, false);
			OpenNijiiroRW.ConfigIni.strSystemSkinSubfolderFullName = OpenNijiiroRW.Skin.GetCurrentSkinSubfolderFullName(true);  // 旧指定のSkinフォルダが消滅していた場合に備える

			ChangeResolution(OpenNijiiroRW.Skin.Resolution[0], OpenNijiiroRW.Skin.Resolution[1]);
		...
		try
		{
			...
			SoundManager = new SoundManager(WindowContext,
				...
				0,
				OpenNijiiroRW.ConfigIni.nASIODevice,
				OpenNijiiroRW.ConfigIni.bUseOSTimer
			);
	...
	private void tExitProcess()
	{
		if (!this.b終了処理完了済み)
		{
			...
			#region [ 現在のステージの終了処理 ]
			//---------------------
			if (OpenNijiiroRW.rCurrentStage != null && OpenNijiiroRW.rCurrentStage.IsActivated)     // #25398 2011.06.07 MODIFY FROM
			{
...
#pragma warning restore SYSLIB0011
			...
			//---------------------
			#endregion
			Trace.TraceInformation("OpenNijiiroRW has closed down successfully.");
			this.b終了処理完了済み = true;
		}
	}
	...
	public void RefreshSkin()
	{
		...
		OpenNijiiroRW.Skin.Dispose();
		OpenNijiiroRW.Skin = null;
		OpenNijiiroRW.Skin = new CSkin(OpenNijiiroRW.ConfigIni.strSystemSkinSubfolderFullName, false);

		OpenNijiiroRW.Tx.DisposeTexture();

		ChangeResolution(OpenNijiiroRW.Skin.Resolution[0], OpenNijiiroRW.Skin.Resolution[1]);

		OpenNijiiroRW.Tx.LoadTexture();

		OpenNijiiroRW.actTextConsole.Activate();
		actTextConsole.CreateManagedResource();
		actTextConsole.CreateUnmanagedResource();
		OpenNijiiroRW.NamePlate.RefleshSkin();
		OpenNijiiroRW.ModalManager.RefleshSkin();
