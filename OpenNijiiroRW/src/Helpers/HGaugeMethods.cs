...
class HGaugeMethods {
	...
	public static void tDrawCompleteGauge(
		...
		if (!OpenNijiiroRW.ConfigIni.SimpleMode) tDrawGaugeFlash(flashTexture, x, y, Opacity, diff, level, currentPercent, gaugeType, scale_x, scale_y);
	...
	public static bool UNSAFE_FastNormaCheck(int player) {
		...
		return tNormaCheck(
			(Difficulty)_dif,
			OpenNijiiroRW.stageSongSelect.rChoosenSong.score[_dif]?.譜面情報.nレベル[_dif] ?? -1,
			tGetGaugeTypeEnum(chara.effect.tGetGaugeType()),
			(float)OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値[player],
	...
	public static float UNSAFE_KillZonePercent(int player) {
		...
		int perfectHits = OpenNijiiroRW.stageGameScreen.CChartScore[player].nGreat;
		int totalHits = dtx.nノーツ数[3];

		// Difficulty
		int _dif = OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[player];
		Difficulty difficulty = (Difficulty)_dif;
		int level = OpenNijiiroRW.stageSongSelect.rChoosenSong.score[_dif]?.譜面情報.nレベル[_dif] ?? -1;
	...
	public static void UNSAFE_DrawGaugeFast(int player, int opacity, int rainbowTextureIndex, int soulFlameIndex) {
		...
		// Set box
		GaugeBox = new int[] { OpenNijiiroRW.Skin.Game_Gauge_Rect[0], OpenNijiiroRW.Skin.Game_Gauge_Rect[1], OpenNijiiroRW.Skin.Game_Gauge_Rect[2], OpenNijiiroRW.Skin.Game_Gauge_Rect[3] };
		...
		int _dif = OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[player];
		Difficulty difficulty = (Difficulty)_dif;
		int level = OpenNijiiroRW.stageSongSelect.rChoosenSong.score[_dif].譜面情報.nレベル[_dif];

		// Current percent
		float currentPercent = (float)OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値[player];

		// Gauge type
		EGaugeType gaugeType = tGetGaugeTypeEnum(chara.effect.tGetGaugeType());

		// Textures
		int _4pGaugeIDX = (OpenNijiiroRW.ConfigIni.nPlayerCount >= 3) ? 1 : 0;
		int _usedGauge = player + 3 * _4pGaugeIDX;
		if (OpenNijiiroRW.P1IsBlue()) _usedGauge = 2;
		_4pGaugeIDX = (OpenNijiiroRW.ConfigIni.nPlayerCount >= 3) ? 2
			: (player == 1) ? 1
			: 0;

		CTexture baseTexture = OpenNijiiroRW.Tx.Gauge_Base[_usedGauge];
		CTexture fillTexture = OpenNijiiroRW.Tx.Gauge[_usedGauge];

		CTexture[] rainbowTextureArr = (new CTexture[][] { OpenNijiiroRW.Tx.Gauge_Rainbow, OpenNijiiroRW.Tx.Gauge_Rainbow_2PGauge, OpenNijiiroRW.Tx.Gauge_Rainbow_Flat })[_4pGaugeIDX];
		CTexture yellowTexture = OpenNijiiroRW.Tx.Gauge_Clear[_4pGaugeIDX];
		CTexture baseNormaTexture = OpenNijiiroRW.Tx.Gauge_Base_Norma[_4pGaugeIDX];
		CTexture killzoneTexture = OpenNijiiroRW.Tx.Gauge_Killzone[_4pGaugeIDX];

		CTexture flashTexture = yellowTexture;
		CTexture clearIcon = (_4pGaugeIDX == 2)
			? null
			: (gaugeType != EGaugeType.NORMAL)
				? OpenNijiiroRW.Tx.Gauge_Killzone[0]
				: OpenNijiiroRW.Tx.Gauge[0];
		CTexture soulLetter = OpenNijiiroRW.Tx.Gauge_Soul;
		CTexture soulFlame = OpenNijiiroRW.Tx.Gauge_Soul_Fire;

		// Rectangles
		Rectangle clearRectHighlight = new Rectangle(
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Rect[0],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Rect[1],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Rect[2],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Rect[3]
		);

		Rectangle clearRect = new Rectangle(
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Clear_Rect[0],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Clear_Rect[1],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Clear_Rect[2],
			OpenNijiiroRW.Skin.Game_Gauge_ClearText_Clear_Rect[3]
		);
	...
	public static void UNSAFE_DrawResultGaugeFast(int player, int shiftPos, int pos, int segmentsDisplayed, int rainbowTextureIndex, int soulFlameIndex, int uioffset_x) {
		...
		// Set box
		GaugeBox = new int[] { OpenNijiiroRW.Skin.Result_Gauge_Rect[0], OpenNijiiroRW.Skin.Result_Gauge_Rect[1], OpenNijiiroRW.Skin.Result_Gauge_Rect[2], OpenNijiiroRW.Skin.Result_Gauge_Rect[3] };
		...
		if (OpenNijiiroRW.ConfigIni.nPlayerCount == 5) {
			...
			soulFire_y = OpenNijiiroRW.Skin.Result_Soul_Fire_Y[pos];
		}
		...
		CTexture baseTexture = OpenNijiiroRW.Tx.Result_Gauge_Base[_usedGauge];
		CTexture fillTexture = OpenNijiiroRW.Tx.Result_Gauge[_usedGauge];

		CTexture[] rainbowTextureArr = OpenNijiiroRW.Tx.Result_Rainbow;
		CTexture yellowTexture = OpenNijiiroRW.Tx.Result_Gauge_Clear;
		CTexture baseNormaTexture = OpenNijiiroRW.Tx.Result_Gauge_Clear_Base;
		CTexture killzoneTexture = OpenNijiiroRW.Tx.Result_Gauge_Killzone;

		CTexture flashTexture = null;
		CTexture clearIcon = (gaugeType != EGaugeType.NORMAL)
			? OpenNijiiroRW.Tx.Result_Gauge_Killzone
			: OpenNijiiroRW.Tx.Result_Gauge[0];

		CTexture soulLetter = OpenNijiiroRW.Tx.Result_Soul_Text;
		CTexture soulFlame = OpenNijiiroRW.Tx.Result_Soul_Fire;

		// Rectangles
		Rectangle clearRectHighlight = new Rectangle(
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Clear_Rect[0],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Clear_Rect[1],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Clear_Rect[2],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Clear_Rect[3]
		);

		Rectangle clearRect = new Rectangle(
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Rect[0],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Rect[1],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Rect[2],
			OpenNijiiroRW.Skin.Result_Gauge_ClearText_Rect[3]
		);
	...
	private static int[] GaugeBox = { OpenNijiiroRW.Skin.Game_Gauge_Rect[0], OpenNijiiroRW.Skin.Game_Gauge_Rect[1], OpenNijiiroRW.Skin.Game_Gauge_Rect[2], OpenNijiiroRW.Skin.Game_Gauge_Rect[3] };
