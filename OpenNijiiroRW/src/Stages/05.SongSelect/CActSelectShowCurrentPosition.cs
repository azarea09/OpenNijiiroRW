...
internal class CActSelectShowCurrentPosition : CActivity
{
	...
	public override void CreateManagedResource()
	{
		...
		{
			this.txScrollBar = OpenNijiiroRW.tテクスチャの生成(pathScrollBar, false);
		}
		if (File.Exists(pathScrollPosition))
		{
			this.txScrollPosition = OpenNijiiroRW.tテクスチャの生成(pathScrollPosition, false);
		}
		base.CreateManagedResource();
	}
	public override void ReleaseManagedResource()
	{
		OpenNijiiroRW.tDisposeSafely(ref this.txScrollBar);
		OpenNijiiroRW.tDisposeSafely(ref this.txScrollPosition);
	...
	public override int Draw()
	{
		if (this.txScrollBar != null)
		{
			...
			int py = OpenNijiiroRW.stageSongSelect.nスクロールバー相対y座標;
			if (py <= 336 && py >= 0)
			{
