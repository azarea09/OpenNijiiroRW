	...
	public override int Draw()
	{
		if (!base.IsDeActivated && this.bIsActivePopupMenu)
		{
			...
			if (OpenNijiiroRW.Tx.Menu_Highlight != null)
			{
				...
				int width = OpenNijiiroRW.Tx.Menu_Highlight.szTextureSize.Width / 2;
				int height = OpenNijiiroRW.Tx.Menu_Highlight.szTextureSize.Height;
				...
				OpenNijiiroRW.Tx.Menu_Highlight.t2D描画(curX, curY, new Rectangle(width, 0, width, height));
			}
			#endregion
			#region [ ソート候補文字列描画 ]
			for (int i = 0; i < lciMenuItems.Length; i++)
