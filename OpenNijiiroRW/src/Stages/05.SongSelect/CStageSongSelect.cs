...
internal class CStageSongSelect : CStage
{
	...
	public void tNotifySelectedSongChange()
	{
		...
		if (rPrevSelectedSong != null)
		{
			...
			if (bchangedBGPath)
				OpenNijiiroRW.tテクスチャの解放(ref txCustomPrevSelectBG);
		...
		this.ct背景スクロール用タイマー = new CCounter(0, (int)(txGenreBack.szTextureSize.Width * scale), 30, OpenNijiiroRW.Timer);
		this.ct背景スクロール用タイマー.CurrentValue = Math.Min(scroll, (int)(txGenreBack.szTextureSize.Width * scale));

		float oldScale = OpenNijiiroRW.Skin.Resolution[1] / (float)txOldGenreBack.szTextureSize.Height;
		this.ctOldBGScroll = new CCounter(0, (int)(txOldGenreBack.szTextureSize.Width * oldScale), 30, OpenNijiiroRW.Timer);
	...
	public override void Activate()
	{
		...
		try
		{
			...
			this.ct背景スクロール用タイマー = new CCounter(0, txGenreBack.szTextureSize.Width, 30, OpenNijiiroRW.Timer);
			this.ctOldBGScroll = new CCounter(0, txOldGenreBack.szTextureSize.Width, 30, OpenNijiiroRW.Timer);

			OpenNijiiroRW.Skin.voiceMenuSongSelect[OpenNijiiroRW.SaveFile]?.tPlay();
			...
			ctCreditAnime = new CCounter(0, 4500, 1, OpenNijiiroRW.Timer);
			ctTimer = new CCounter(0, 100, 1000, OpenNijiiroRW.Timer);
	...
	public override int Draw()
	{
		if (!base.IsDeActivated)
		{
				...
				else if (this.rNowSelectedSong.nodeType == CSongListNode.ENodeType.SCORE)
				{
					...
					else
					{
						if (OpenNijiiroRW.stageSongSelect.n現在選択中の曲の難易度 == (int)Difficulty.Dan)
							...
							OpenNijiiroRW.Tx.SongSelect_Song_Panel[3]?.t2D描画(0, 0);
						else
							OpenNijiiroRW.Tx.SongSelect_Song_Panel[1]?.t2D描画(0, 0);
					}
				}
			...
			OpenNijiiroRW.Tx.SongSelect_Header?.t2D描画(0, 0);
			OpenNijiiroRW.Tx.SongSelect_Footer?.t2D描画(0, 0);
			...
			tSongNumberDraw(OpenNijiiroRW.Skin.SongSelect_SongNumber_X[0], OpenNijiiroRW.Skin.SongSelect_SongNumber_Y[0], NowSong);
			tSongNumberDraw(OpenNijiiroRW.Skin.SongSelect_SongNumber_X[1], OpenNijiiroRW.Skin.SongSelect_SongNumber_Y[1], MaxSong);
			...
			int dyGameMode = OpenNijiiroRW.actTextConsole.fontHeight;
			int yGameMode = 0;
			if (OpenNijiiroRW.ConfigIni.bTokkunMode)
				OpenNijiiroRW.actTextConsole.Print(0, yGameMode, CTextConsole.EFontType.White, "GAME: TRAINING MODE");
			yGameMode += dyGameMode;
			if (OpenNijiiroRW.ConfigIni.eGameMode == EGame.Survival)
				OpenNijiiroRW.actTextConsole.Print(0, yGameMode, CTextConsole.EFontType.White, "GAME: SURVIVAL");
			if (OpenNijiiroRW.ConfigIni.eGameMode == EGame.SurvivalHard)
				OpenNijiiroRW.actTextConsole.Print(0, yGameMode, CTextConsole.EFontType.White, "GAME: SURVIVAL HARD");
			yGameMode += dyGameMode;
			if (OpenNijiiroRW.ConfigIni.bSuperHard)
				OpenNijiiroRW.actTextConsole.Print(0, yGameMode, CTextConsole.EFontType.Cyan, "SUPER HARD MODE : ON");
			...
			for (int player = 0; player < OpenNijiiroRW.ConfigIni.nPlayerCount; player++)
			{
				...
				int chara_x = OpenNijiiroRW.Skin.SongSelect_NamePlate_X[player] + OpenNijiiroRW.Tx.NamePlateBase.szTextureSize.Width / 2;
				int chara_y = OpenNijiiroRW.Skin.SongSelect_NamePlate_Y[player];
			...
			if (OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.bIsDifficltSelect)
			{
				...
				if (OpenNijiiroRW.ConfigIni.nPlayerCount > 1 && OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[1] >= 2)
					currentPads[1] = OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[1] - 2;
				if (OpenNijiiroRW.ConfigIni.nPlayerCount > 1 && OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[2] >= 2)
					currentPads[2] = OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[2] - 2;
				if (OpenNijiiroRW.ConfigIni.nPlayerCount > 1 && OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[3] >= 2)
					currentPads[3] = OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[3] - 2;
				if (OpenNijiiroRW.ConfigIni.nPlayerCount > 1 && OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[4] >= 2)
					currentPads[4] = OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen.n現在の選択行[4] - 2;
			}
			...
			for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++)
			{
				...
				OpenNijiiroRW.Tx.SongSelect_Table[currentPads[i]]?.t2D描画(OpenNijiiroRW.Skin.SongSelect_Table_X[i], OpenNijiiroRW.Skin.SongSelect_Table_Y[i]);
			...
			for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++)
			{
				...
				if (OpenNijiiroRW.SaveFileInstances[p].data.Medals >= 0)
					tBoardNumberDraw(OpenNijiiroRW.Skin.SongSelect_BoardNumber_X[i][11], OpenNijiiroRW.Skin.SongSelect_BoardNumber_Y[i][11], (int)OpenNijiiroRW.SaveFileInstances[p].data.Medals);
				...
				if (song != null && song.nodeType == CSongListNode.ENodeType.SCORE)
				{
					...
					if (score != null)
					{
						...
						OpenNijiiroRW.Tx.SongSelect_High_Score?.t2D中心基準描画(OpenNijiiroRW.Skin.SongSelect_High_Score_X[i], OpenNijiiroRW.Skin.SongSelect_High_Score_Y[i]);
						...
						if (this.n現在選択中の曲の難易度 <= (int)Difficulty.Edit)
						{
							...
							__tex.t2D中心基準描画(
								OpenNijiiroRW.Skin.SongSelect_High_Score_Difficulty_Cymbol_X[i],
								OpenNijiiroRW.Skin.SongSelect_High_Score_Difficulty_Cymbol_Y[i],
								new Rectangle(table * width, 0, width, height));
						}

						tBoardNumberDraw(OpenNijiiroRW.Skin.SongSelect_BoardNumber_X[i][12], OpenNijiiroRW.Skin.SongSelect_BoardNumber_Y[i][12], displayedScore);
					}
			...
			if (base.ePhaseID == CStage.EPhase.Common_NORMAL)
			{
				...
				if (this.actSongList.isContextBoxOpened == true)
				{
					...
					if (__done == true)
					{
						...
						else if (this.actSongList.latestContext == eMenuContext.Random)
						{
							...
							this.actSongList.ctBarFlash.Start(0, 2700, OpenNijiiroRW.Skin.SongSelect_Box_Opening_Interval, OpenNijiiroRW.Timer);
							this.actSongList.ctDifficultyIn.Start(0, 3200, OpenNijiiroRW.Skin.SongSelect_Box_Opening_Interval, OpenNijiiroRW.Timer);
														...
														if (SongToUnlock != null)
														{
															...
															if (response.Item1)
															{
																...
																if (SongToUnlock.unlockConditions is CUnlockCM)
																	OpenNijiiroRW.SaveFileInstances[OpenNijiiroRW.SaveFile].tSpendCoins(SongToUnlock.unlockConditions.Values[0]);
											...
											case CSongListNode.ENodeType.BOX:
												{
													...
													OpenNijiiroRW.Skin.soundDecideSFX.tPlay();
													this.actSongList.ctBarFlash.Start(0, 2700, OpenNijiiroRW.Skin.SongSelect_Box_Opening_Interval, OpenNijiiroRW.Timer);
													this.actSongList.ctBoxOpen.Start(200, 2700, OpenNijiiroRW.Skin.SongSelect_Box_Opening_Interval * 1.3f, OpenNijiiroRW.Timer);
													this.actSongList.bBoxOpen = true;
													//this.ctChara_Select.t開始(0, TJAPlayer3.Tx.SongSelect_Chara_Select.Length - 1, 1000 / 45, TJAPlayer3.Timer);
													CMenuCharacter.tMenuResetTimer(CMenuCharacter.ECharacterAnimation.SELECT);
												}
				...
				KeyboardSoundGroupLevelControlHandler.Handle(
					OpenNijiiroRW.InputManager.Keyboard, OpenNijiiroRW.SoundGroupLevelController, OpenNijiiroRW.Skin, true);
				#endregion
			...
			if (OpenNijiiroRW.ConfigIni.nPlayerCount == 1)
			{
				...
				OpenNijiiroRW.Tx.SongSelect_Credit.Opacity = opacity;
    ...
    */
	...
	public void tBoardNumberDraw(int x, int y, int num)
	{
		...
		for (int j = 0; j < nums.Length; j++)
		{
			...
			float width = OpenNijiiroRW.Tx.SongSelect_BoardNumber.sz画像サイズ.Width / 10.0f;
			float height = OpenNijiiroRW.Tx.SongSelect_BoardNumber.sz画像サイズ.Height;

			OpenNijiiroRW.Tx.SongSelect_BoardNumber.t2D描画(_x, _y, new RectangleF(width * nums[j], 0, width, height));
		}
	}
	...
	private void tSongNumberDraw(int x, int y, int num)
	{
		...
		for (int j = 0; j < nums.Length; j++)
		{
			...
			float width = OpenNijiiroRW.Tx.SongSelect_Song_Number.sz画像サイズ.Width / 10.0f;
			float height = OpenNijiiroRW.Tx.SongSelect_Song_Number.sz画像サイズ.Height;

			OpenNijiiroRW.Tx.SongSelect_Song_Number.t2D描画(_x, _y, new RectangleF(width * nums[j], 0, width, height));
		}
	}
	...
	private void tTimerDraw(int num)
	{
		...
		for (int j = 0; j < nums.Length; j++)
		{
			if (OpenNijiiroRW.ConfigIni.bEnableCountdownTimer)
			{
				...
				float width = OpenNijiiroRW.Tx.SongSelect_Timer.sz画像サイズ.Width / 10.0f;
				float height = OpenNijiiroRW.Tx.SongSelect_Timer.sz画像サイズ.Height / 2.0f;

				OpenNijiiroRW.Tx.SongSelect_Timer.t2D描画(x, y, new RectangleF(width * nums[j], 0, width, height));
			}
		}
	}
	...
	private void tカーソルを下へ移動する()
	{
		...
		if (this.actSongList.ctBarOpen.CurrentValue >= 200 || this.ctBackgroundFade.CurrentValue >= 600 - 255)
		{
			...
			OpenNijiiroRW.stageSongSelect.OldBgColor = this.rNowSelectedSong.BgColor;
		}
