...
internal class CActPlayOption : CActivity {
	...
	public override void Activate() {
		...
		hsInfo = OpenNijiiroRW.Skin.hsHitSoundsInformations;
	...
	public int On進行描画(int nPlayers, ReadOnlySpan<bool> bOptions) {
		...
		var act難易度 = OpenNijiiroRW.stageSongSelect.actDifficultySelectionScreen;
		var danAct = OpenNijiiroRW.stageDanSongSelect.段位挑戦選択画面;
		...
		bool centerMenu = (OpenNijiiroRW.Pad.bPressedDGB(EPad.Decide) ||
		...
		bool upMenu = (OpenNijiiroRW.InputManager.Keyboard.KeyPressed((int)SlimDXKeys.Key.UpArrow));
		...
		for (int player = 0; player < nPlayers; ++player) {
			...
			if (!ctOpen.IsTicked)
				ctOpen.Start(0, 50, 6, OpenNijiiroRW.Timer);
			...
			var pos = player % 2;
			var _shift = pos == 1 ? (OpenNijiiroRW.Tx.Difficulty_Option.szTextureSize.Width / 2) : 0;
			var _rect = new Rectangle(_shift, 0, OpenNijiiroRW.Tx.Difficulty_Option.szTextureSize.Width / 2, OpenNijiiroRW.Tx.Difficulty_Option.szTextureSize.Height);

			OpenNijiiroRW.Tx.Difficulty_Option.t2D描画(_shift, y, _rect);
			...
			if (!ctClose.IsTicked) {
				...
				switch (player) {
					case 0:
						...
						_centerDrum = _centerDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LRed) || OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RRed));
						break;
					case 1:
						_rightDrum = _rightDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RBlue2P));
						_leftDrum = _leftDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LBlue2P));
						_centerDrum = _centerDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LRed2P) || OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RRed2P));
						break;
					case 2:
					...
					case 3:
						...
						_leftDrum = _leftDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LBlue4P));
						_centerDrum = _centerDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LRed4P) || OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RRed4P));
						break;
					case 4:
						_rightDrum = _rightDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RBlue5P));
						_leftDrum = _leftDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LBlue5P));
						_centerDrum = _centerDrum || (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.LRed5P) || OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.RRed5P));
						break;
				}
	...
	public void Init(int player) {
		...
		int speed = OpenNijiiroRW.ConfigIni.nScrollSpeed[actual];
		...
		nStealth = (int)OpenNijiiroRW.ConfigIni.eSTEALTH[actual];
		...
		nTiming = OpenNijiiroRW.ConfigIni.nTimingZones[actual];

		#endregion

		#region [Just]

		nJust = OpenNijiiroRW.ConfigIni.bJust[actual];

		#endregion

		#region [GameType]

		nGameType = (int)OpenNijiiroRW.ConfigIni.nGameType[actual];

		#endregion

		#region [Fun mods]

		nFunMods = (int)OpenNijiiroRW.ConfigIni.nFunMods[actual];

		#endregion

		#region [ GameMode ]

		if (OpenNijiiroRW.ConfigIni.bTokkunMode == true)
			nGameMode = 1;
		else
			nGameMode = 0;
		...
		bool _auto = OpenNijiiroRW.ConfigIni.bAutoPlay[player];
		...
		nOtoiro = Math.Min(txOtoiro.Length - 1, OpenNijiiroRW.ConfigIni.nHitSounds[actual]);

		#endregion

		#region [ Song speed ]

		nSongSpeed = Math.Max(0, Math.Min(txSongSpeed.Length - 1, (OpenNijiiroRW.ConfigIni.nSongSpeed / 2) - 5));
	...
	public void Decision(int player) {
		...
		OpenNijiiroRW.ConfigIni.eSTEALTH[actual] = (EStealthMode)nStealth;
		...
		OpenNijiiroRW.ConfigIni.nTimingZones[actual] = nTiming;

		#endregion

		#region [Just]

		OpenNijiiroRW.ConfigIni.bJust[actual] = nJust;

		#endregion

		#region [GameType]

		OpenNijiiroRW.ConfigIni.nGameType[actual] = (EGameType)nGameType;

		#endregion

		#region [Fun mods]

		OpenNijiiroRW.ConfigIni.nFunMods[actual] = (EFunMods)nFunMods;
		...
		OpenNijiiroRW.ConfigIni.nHitSounds[actual] = nOtoiro;
		...
		OpenNijiiroRW.ConfigIni.nSongSpeed = (nSongSpeed + 5) * 2;
