...
internal class CActSelectQuickConfig : CActSelectPopupMenu {
	...
	private List<CItemBase> MakeListCItemBase(int nConfigSet, int nInst) {
		...
		l.Add(new CItemInteger(CLangManager.LangInstance.GetString("MOD_SPEED"), 0, 1999, OpenNijiiroRW.ConfigIni.nScrollSpeed[OpenNijiiroRW.SaveFile],
			""));
		#endregion
		#region [ 共通 Dark/Risky/PlaySpeed ]
		l.Add(new CItemInteger(CLangManager.LangInstance.GetString("MOD_SONGSPEED"), 5, 400, OpenNijiiroRW.ConfigIni.nSongSpeed,
			""));
		#endregion
		#region [ 個別 Sud/Hid ]
		l.Add(new CItemList(CLangManager.LangInstance.GetString("MOD_RANDOM"), CItemBase.EPanelType.Normal, (int)OpenNijiiroRW.ConfigIni.eRandom[OpenNijiiroRW.SaveFile],
			"",
		...
		l.Add(new CItemList(CLangManager.LangInstance.GetString("MOD_HIDE"), CItemBase.EPanelType.Normal, (int)OpenNijiiroRW.ConfigIni.eSTEALTH[OpenNijiiroRW.SaveFile],
			"",
		...
		l.Add(new CItemList(CLangManager.LangInstance.GetString("SETTINGS_GAME_SURVIVAL"), CItemBase.EPanelType.Normal, (int)OpenNijiiroRW.ConfigIni.eGameMode,
			"",
			new string[] { CLangManager.LangInstance.GetString("MOD_SWITCH_OFF"), "TYPE-A", "TYPE-B" }));

		l.Add(new CItemList(CLangManager.LangInstance.GetString("SETTINGS_GAME_SHINUCHI"), CItemBase.EPanelType.Normal, OpenNijiiroRW.ConfigIni.ShinuchiMode ? 1 : 0, "", "", new string[] { CLangManager.LangInstance.GetString("MOD_SWITCH_OFF"), CLangManager.LangInstance.GetString("MOD_SWITCH_ON") }));

		#endregion
		#region [ 共通 SET切り替え/More/Return ]
		l.Add(new CItemInteger(CLangManager.LangInstance.GetString("SETTINGS_SYSTEM_PLAYERCOUNT"), 1, 5, OpenNijiiroRW.ConfigIni.nPlayerCount, ""));
	...
	public override void tEnter押下Main(int nSortOrder) {
		switch (n現在の選択行) {
			...
			case (int)EOrder.PlaySpeed:
				OpenNijiiroRW.ConfigIni.nSongSpeed = (int)GetObj現在値((int)EOrder.PlaySpeed);
				break;
			case (int)EOrder.Random:
				OpenNijiiroRW.ConfigIni.eRandom[OpenNijiiroRW.SaveFile] = (ERandomMode)GetIndex((int)EOrder.Random);
				break;
			case (int)EOrder.Stealth:
				OpenNijiiroRW.ConfigIni.eSTEALTH[OpenNijiiroRW.SaveFile] = (EStealthMode)GetIndex((int)EOrder.Stealth);
				break;
			case (int)EOrder.GameMode:
				...
				OpenNijiiroRW.ConfigIni.eGameMode = game;
				break;
			case (int)EOrder.ShinuchiMode:
				OpenNijiiroRW.ConfigIni.ShinuchiMode = !OpenNijiiroRW.ConfigIni.ShinuchiMode;
				break;
			case (int)EOrder.PlayerCount:
				OpenNijiiroRW.ConfigIni.nPlayerCount = (int)GetObj現在値((int)EOrder.PlayerCount);
				break;
