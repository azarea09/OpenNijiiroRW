	...
	public override void Activate()
	{
		...
		try
		{
			...
			if (OpenNijiiroRW.ConfigIsNew)
			{
				...
				langSelectOffset = new int[2] { RenderSurfaceSize.Width / 2, (RenderSurfaceSize.Height - langList.Select(tex => tex.szTextureSize.Height).Sum() - langSelectTitle.szTextureSize.Height) / 2 };
			}
	...
	public override int Draw()
	{
		if (!base.IsDeActivated)
		{
			if (base.IsFirstDraw)
			{
				...
				this.list進行文字列.Add("OpenNijiiroRW forked TJAPlayer3-Develop-ReWrite(@TouhouRenren)");
				this.list進行文字列.Add("OpenNijiiroRW edited by 0AuBSQ");
				this.list進行文字列.Add("");
			...
			else
			{
				...
				OpenNijiiroRW.Tx.Readme.t2D描画(0, 0);
			}
