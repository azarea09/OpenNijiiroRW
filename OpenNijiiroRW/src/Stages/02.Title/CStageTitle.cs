...
internal class CStageTitle : CStage {
	...
	public override int Draw() {
		if (!base.IsDeActivated) {
				...
				*/
				...
				if (!bSaveIsLoading) {
					...
					if (OpenNijiiroRW.Pad.bPressed(EInstrumentPad.Drums, EPad.Decide) ||
						...
						this.ctSaveLoading.Start(0, 600, 1, OpenNijiiroRW.Timer);
					...
					if (bプレイヤーエントリー && this.ctSaveLoaded.IsEnded) {
						if (n現在の選択行プレイヤーエントリー == 0 || n現在の選択行プレイヤーエントリー == 2) {
							if (!bプレイヤーエントリー決定) {
								...
								bプレイヤーエントリー決定 = true;
								OpenNijiiroRW.PlayerSide = (n現在の選択行プレイヤーエントリー == 2) ? 1 : 0;
								if (OpenNijiiroRW.PlayerSide == 1)
							...
							OpenNijiiroRW.Skin.SoundBanapas.bPlayed = false;
				...
				if (ctエントリーバー決定点滅.CurrentValue >= 1055) {
					if (!bモード選択) {
						...
						*/
						...
						if (OpenNijiiroRW.Skin.voiceTitleSanka[OpenNijiiroRW.SaveFile] != null && !OpenNijiiroRW.Skin.voiceTitleSanka[OpenNijiiroRW.SaveFile].bPlayed)
							OpenNijiiroRW.Skin.voiceTitleSanka[OpenNijiiroRW.SaveFile]?.tPlay();

						ctキャライン.Start(0, 180, 2, OpenNijiiroRW.Timer);
						ctBarAnimeIn.Start(0, 1295, 1, OpenNijiiroRW.Timer);
						bモード選択 = true;
					}
				}
			}
			...
			if (bSaveFileLoaded == false) {
				...
				if (!bSaveIsLoading) {
					...
					if (this.ctコインイン待機.CurrentValue <= 255)
						OpenNijiiroRW.Tx.Entry_Bar_Text.Opacity = this.ctコインイン待機.CurrentValue;
					else if (this.ctコインイン待機.CurrentValue <= 2000 - 355)
						OpenNijiiroRW.Tx.Entry_Bar_Text.Opacity = 255;
					else
						OpenNijiiroRW.Tx.Entry_Bar_Text.Opacity = 255 - (this.ctコインイン待機.CurrentValue - (2000 - 355));
					...
					if (this.ctSaveLoaded.CurrentValue <= 1000 && this.ctSaveLoadingFailed.CurrentValue <= 1128) {
						if (bSaveIsLoading) {
							...
							for (int i = 0; i < OpenNijiiroRW.Skin.Resolution[0] / OpenNijiiroRW.Tx.Tile_Black.szTextureSize.Width + 1; i++)
								for (int j = 0; j < OpenNijiiroRW.Skin.Resolution[1] / OpenNijiiroRW.Tx.Tile_Black.szTextureSize.Height + 1; j++)
							...
							if (OpenNijiiroRW.Tx.Banapas_Load[2] != null) {
								...
								for (int i = 0; i < OpenNijiiroRW.Skin.Title_LoadingPinInstances; i++) {
									...
									OpenNijiiroRW.Tx.Banapas_Load[2].t2D拡大率考慮中央基準描画(
										OpenNijiiroRW.Skin.Title_LoadingPinBase[0] + OpenNijiiroRW.Skin.Title_LoadingPinDiff[0] * i,
										OpenNijiiroRW.Skin.Title_LoadingPinBase[1] + OpenNijiiroRW.Skin.Title_LoadingPinDiff[1] * i,
										new Rectangle(step
													  * (_stamp >= i * cycle
														  ? _stamp <= (i + 1) * cycle
															  ? (_stamp + i * cycle) / (cycle / OpenNijiiroRW.Skin.Title_LoadingPinFrameCount)
															  : 0
														  : 0),
											0,
											step,
											OpenNijiiroRW.Tx.Banapas_Load[2].szTextureSize.Height));
								}
							}
						...
						if (bSaveIsLoading) {
							...
							if (!OpenNijiiroRW.Skin.SoundBanapas.bPlayed)
								OpenNijiiroRW.Skin.SoundBanapas.tPlay();
			...
			if (bプレイヤーエントリー) {
				...
				OpenNijiiroRW.Tx.Entry_Player[0].Opacity = alpha;
				OpenNijiiroRW.Tx.Entry_Player[1].Opacity = alpha;
				...
				*/
				...
				OpenNijiiroRW.Tx.Entry_Player[0].t2D描画(0, 0);

				//Chara_Entry[this.ctキャラエントリーループ.n現在の値].t2D描画(485, 140);

				int _actual = OpenNijiiroRW.GetActualPlayer(0);

				int _charaId = OpenNijiiroRW.SaveFileInstances[_actual].data.Character;

				int chara_x = OpenNijiiroRW.Skin.Title_Entry_NamePlate[0] + OpenNijiiroRW.Tx.NamePlateBase.szTextureSize.Width / 2;
				int chara_y = OpenNijiiroRW.Skin.Title_Entry_NamePlate[1];
				...
				*/
				...
				OpenNijiiroRW.Tx.Entry_Player[2].Opacity = ctエントリーバー決定点滅.CurrentValue >= 800 ? 255 - (ctエントリーバー決定点滅.CurrentValue - 800) : (this.ctSaveLoaded.CurrentValue - 3400) - (this.ctエントリーバー点滅.CurrentValue <= 255 ? this.ctエントリーバー点滅.CurrentValue : 255 - (this.ctエントリーバー点滅.CurrentValue - 255));
				OpenNijiiroRW.Tx.Entry_Player[2].t2D描画(OpenNijiiroRW.Skin.Title_Entry_Player_Select_X[n現在の選択行プレイヤーエントリー], OpenNijiiroRW.Skin.Title_Entry_Player_Select_Y[n現在の選択行プレイヤーエントリー],
					new RectangleF(OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[0][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][0],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[0][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][1],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[0][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][2],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[0][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][3]
					));

				OpenNijiiroRW.Tx.Entry_Player[2].Opacity = alpha;
				OpenNijiiroRW.Tx.Entry_Player[2].t2D描画(OpenNijiiroRW.Skin.Title_Entry_Player_Select_X[n現在の選択行プレイヤーエントリー], OpenNijiiroRW.Skin.Title_Entry_Player_Select_Y[n現在の選択行プレイヤーエントリー],
					new RectangleF(OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[1][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][0],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[1][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][1],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[1][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][2],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[1][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][3]
					));

				OpenNijiiroRW.Tx.Entry_Player[1].t2D描画(0, 0);
				...
				OpenNijiiroRW.Tx.Entry_Player[2].Opacity = Opacity;
				OpenNijiiroRW.Tx.Entry_Player[2].t2D描画(OpenNijiiroRW.Skin.Title_Entry_Player_Select_X[n現在の選択行プレイヤーエントリー], OpenNijiiroRW.Skin.Title_Entry_Player_Select_Y[n現在の選択行プレイヤーエントリー],
					new RectangleF(OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[2][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][0],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[2][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][1],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[2][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][2],
						OpenNijiiroRW.Skin.Title_Entry_Player_Select_Rect[2][n現在の選択行プレイヤーエントリー == 1 ? 1 : 0][3]
					));

				Opacity = ctエントリーバー決定点滅.CurrentValue >= 800 ? 255 - (ctエントリーバー決定点滅.CurrentValue - 800) : (this.ctSaveLoaded.CurrentValue - 3400);
				if (Opacity > 0)
					OpenNijiiroRW.NamePlate.tNamePlateDraw(OpenNijiiroRW.Skin.Title_Entry_NamePlate[0], OpenNijiiroRW.Skin.Title_Entry_NamePlate[1], 0, true, Opacity);
			}
			...
			if (bモード選択) {
				...
				for (int player = 0; player < OpenNijiiroRW.ConfigIni.nPlayerCount; player++) {
					...
					int chara_x = (int)CharaX + OpenNijiiroRW.Skin.SongSelect_NamePlate_X[player] + OpenNijiiroRW.Tx.NamePlateBase.szTextureSize.Width / 2;
					int chara_y = OpenNijiiroRW.Skin.SongSelect_NamePlate_Y[player] - (int)CharaY;

					int puchi_x = chara_x + OpenNijiiroRW.Skin.Adjustments_MenuPuchichara_X[player];
					int puchi_y = chara_y + OpenNijiiroRW.Skin.Adjustments_MenuPuchichara_Y[player];
				...
				if (ctBarAnimeIn.CurrentValue >= (int)(16 * 16.6f)) {
					...
					for (int i = 0; i < usedMenusCount; i++) {
						...
						if ((_menu._1pRestricted == true && OpenNijiiroRW.ConfigIni.nPlayerCount > 1)
							...
							TitleTextureKey.ResolveTitleTexture(_menu.ttkBoxText, OpenNijiiroRW.Skin.Title_VerticalText, true).color4 = CConversion.ColorToColor4(Color.White);
							TitleTextureKey.ResolveTitleTexture(_menu.ttkTitle, OpenNijiiroRW.Skin.Title_VerticalText).color4 = CConversion.ColorToColor4(Color.White);
						}
						...
						if (usedMenusPos[i] == 1 && ctBarMove.CurrentValue >= 150) {
							...
							float barAnime = OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move[0] +
											 (barAnimef * (OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move[1] - OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move[0]));

							float barAnimeX = OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move_X[0] +
											  (barAnimef * (OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move_X[1] - OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Move_X[0]));

							float overlayAnime = OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move[0] +
												 (barAnimef * (OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move[1] - OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move[0]));

							float overlayAnimeX = OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move_X[0] +
												  (barAnimef * (OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move_X[1] - OpenNijiiroRW.Skin.Title_ModeSelect_Overlay_Move_X[0]));
							...
							if (_bar != null) {
								...
								_bar.t2D描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_X[0] - (OpenNijiiroRW.Skin.Title_VerticalBar ? barAnimeX : 0),
									...
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[0][0],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[0][1],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[0][2],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[0][3]));
								_bar.t2D描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_X[1] + (OpenNijiiroRW.Skin.Title_VerticalBar ? barAnimeX : 0),
									OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Y[1] + (OpenNijiiroRW.Skin.Title_VerticalBar ? 0 : barAnime),
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[1][0],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[1][1],
								...
								_bar.t2D拡大率考慮中央基準描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_X[2], OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Y[2],
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[2][0],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[2][1],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[2][2],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Rect[2][3]));
							}
							...
							if (OpenNijiiroRW.Tx.ModeSelect_Bar[CMainMenuTab.__MenuCount] != null) {
								...
								_overlap.t2D描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_X[0], OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Y[0],
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[0][0],
										...
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[0][2],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[0][3]));
								_overlap.t2D描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_X[1] + (OpenNijiiroRW.Skin.Title_VerticalBar ? overlayAnimeX : 0),
									OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Y[1] + (OpenNijiiroRW.Skin.Title_VerticalBar ? 0 : overlayAnime),
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[1][0],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[1][1],
								...
								_overlap.t2D拡大率考慮上中央基準描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_X[2], OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Y[2],
									new Rectangle(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[2][0],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[2][1],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[2][2],
										OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Overlay_Rect[2][3]));
							...
							if (_chara != null) {
								...
								_chara.t2D中心基準描画(OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Chara_X[1] + anime, OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Chara_Y[1],
									new Rectangle(_chara.szTextureSize.Width / 2, 0, _chara.szTextureSize.Width / 2, _chara.szTextureSize.Height));
							}

							TitleTextureKey.ResolveTitleTexture(_menu.ttkTitle, OpenNijiiroRW.Skin.Title_VerticalText)?.t2D中心基準描画(
								OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Title[0] + (OpenNijiiroRW.Skin.Title_ModeSelect_Bar_Center_Title_Move_X * BarAnimeCount),
							...
							TitleTextureKey.ResolveTitleTexture(_menu.ttkTitle, OpenNijiiroRW.Skin.Title_VerticalText)?.t2D中心基準描画(pos.X + BarAnimeX - BarMoveX + OpenNijiiroRW.Skin.Title_ModeSelect_Title_Offset[0], pos.Y + BarAnimeY - BarMoveY + OpenNijiiroRW.Skin.Title_ModeSelect_Title_Offset[1]);
						}
					}
				}
			...
			string strCreator = "https://github.com/0AuBSQ/OpenNijiiroRW";
			AssemblyName asmApp = Assembly.GetExecutingAssembly().GetName();
			int dy = OpenNijiiroRW.actTextConsole.fontHeight;
			int y = 4;
			y = OpenNijiiroRW.actTextConsole.Print(4, y, CTextConsole.EFontType.White, asmApp.Name + " Ver." + OpenNijiiroRW.VERSION + " (" + strCreator + ")").y;
			y += dy + 4;
			y = OpenNijiiroRW.actTextConsole.Print(4, y, CTextConsole.EFontType.White, "Skin:" + OpenNijiiroRW.Skin.Skin_Name + " Ver." + OpenNijiiroRW.Skin.Skin_Version + " (" + OpenNijiiroRW.Skin.Skin_Creator + ")").y;
			y += dy + 4;
			//CDTXMania.act文字コンソール.tPrint(4, 24, C文字コンソール.Eフォント種別.白, strSubTitle);
			OpenNijiiroRW.actTextConsole.Print(4, y, CTextConsole.EFontType.White, "DEBUG BUILD");
			y = OpenNijiiroRW.Skin.Resolution[1] - 8 - dy;
			OpenNijiiroRW.actTextConsole.Print(4, y, CTextConsole.EFontType.White, "TJAPlayer3 forked TJAPlayer2 forPC(kairera0467)");
	...
	private void SkipSaveFileStep() {
		if (bSaveFileLoaded == true) {
			...
			OpenNijiiroRW.Skin.SoundBanapas.bPlayed = true;
			//TJAPlayer3.Skin.soundsanka.bPlayed = true;

			if (OpenNijiiroRW.Skin.voiceTitleSanka[OpenNijiiroRW.SaveFile] != null)
				OpenNijiiroRW.Skin.voiceTitleSanka[OpenNijiiroRW.SaveFile].bPlayed = true;
		}
	}
	...
	private void UnloadSaveFile() {
		...
		this.ctエントリーバー点滅 = new CCounter(0, 510, 2, OpenNijiiroRW.Timer);
