...
class CActSelect段位リスト : CStage
{
	...
	public override void Activate()
	{
		...
		ctDaniIn = new CCounter(0, 6000, 1, OpenNijiiroRW.Timer);

		ctDanTick = new CCounter(0, 510, 3, OpenNijiiroRW.Timer);
		...
		listSongs = OpenNijiiroRW.Songs管理.list曲ルート_Dan;
		tUpdateSongs();
	...
	public override void CreateManagedResource()
	{
		...
		this.pfExamFont = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.DaniSelect_Font_Exam_Size);
	...
	public override int Draw()
	{
		...
		int tickWidth = OpenNijiiroRW.Tx.Dani_Plate.szTextureSize.Width / 7;
		int tickHeight = OpenNijiiroRW.Tx.Dani_Plate.szTextureSize.Height;
		...
		for (int idx = -13; idx < 14; idx++)
		{
			...
			int xPos = OpenNijiiroRW.Skin.DaniSelect_Plate[0] + (idx == 0 ? OpenNijiiroRW.Skin.DaniSelect_Plate_Center_Move[0] : 0) + idx * OpenNijiiroRW.Skin.DaniSelect_Plate_Move[0];
			int yPos = OpenNijiiroRW.Skin.DaniSelect_Plate[1] + (idx == 0 ? OpenNijiiroRW.Skin.DaniSelect_Plate_Center_Move[1] : 0) + idx * OpenNijiiroRW.Skin.DaniSelect_Plate_Move[1];
			...
			OpenNijiiroRW.Tx.Dani_Plate?.tUpdateColor4(CConversion.ColorToColor4(Color.White));
			...
			if (stバー情報[currentSong].eノード種別 == CSongListNode.ENodeType.SCORE)
				TitleTextureKey.ResolveTitleTextureTate(stバー情報[currentSong].ttkタイトル[stバー情報[currentSong].ttkタイトル.Length - 1])
					.t2D拡大率考慮上中央基準描画(xPos + OpenNijiiroRW.Skin.DaniSelect_Plate_Title_Offset[0], yPos + OpenNijiiroRW.Skin.DaniSelect_Plate_Title_Offset[1]);
	...
	public static void tDisplayDanPlate(CTexture givenPlate, STバー情報? songNode, int x, int y)
	{
		...
		else
		{
			...
			OpenNijiiroRW.Tx.Dani_DanPlates_Back?.t2D中心基準描画(x, y, new Rectangle(
				unit * danTick,
				0,
				unit,
				OpenNijiiroRW.Tx.Dani_DanPlates_Back.szTextureSize.Height
			));
			OpenNijiiroRW.Tx.Dani_DanPlates?.t2D中心基準描画(x, y, new Rectangle(
				unit * danTick,
				0,
				unit,
				OpenNijiiroRW.Tx.Dani_DanPlates.szTextureSize.Height
			));

			if (pfDanPlateTitle == null)
				pfDanPlateTitle = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.DaniSelect_DanPlateTitle_Size);
			...
			TitleTextureKey.ResolveTitleTextureTate(ttkTmp).t2D中心基準描画(x + OpenNijiiroRW.Skin.DaniSelect_DanPlateTitle_Offset[0], y + OpenNijiiroRW.Skin.DaniSelect_DanPlateTitle_Offset[1]);
		}
	}
			...
			case CSongListNode.ENodeType.SCORE:
				{
					...
					else
					{
						...
						OpenNijiiroRW.Tx.Dani_Bar_Center.t2D描画(scroll + Anime, 0);

						// Bar sides
						OpenNijiiroRW.Tx.Dani_DanSides.t2D描画((int)(scroll + Anime) + OpenNijiiroRW.Skin.DaniSelect_DanSides_X[0], OpenNijiiroRW.Skin.DaniSelect_DanSides_Y[0], new Rectangle(
							unit * danTick,
							0,
							unit,
							OpenNijiiroRW.Tx.Dani_DanSides.szTextureSize.Height
						));

						OpenNijiiroRW.Tx.Dani_DanSides.t2D左右反転描画((int)(scroll + Anime) + OpenNijiiroRW.Skin.DaniSelect_DanSides_X[1], OpenNijiiroRW.Skin.DaniSelect_DanSides_Y[1], new Rectangle(
							unit * danTick,
							0,
							unit,
							OpenNijiiroRW.Tx.Dani_DanSides.szTextureSize.Height
					...
					CActSelect段位リスト.tDisplayDanPlate(stバー情報[currentSong].txDanPlate, stバー情報[currentSong], (int)(scroll + Anime) + OpenNijiiroRW.Skin.DaniSelect_DanPlate[0], OpenNijiiroRW.Skin.DaniSelect_DanPlate[1]);
					...
					if (currentRank >= 0)
					{
						...
						OpenNijiiroRW.Tx.DanResult_Rank.t2D拡大率考慮中央基準描画(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Rank[0], OpenNijiiroRW.Skin.DaniSelect_Rank[1], new Rectangle(rank_width * (currentRank + 1), 0, rank_width, rank_height));
					}
					...
					OpenNijiiroRW.Tx.Dani_Bloc[2]?.t2D描画(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Bloc2[0], OpenNijiiroRW.Skin.DaniSelect_Bloc2[1]);

					if (stバー情報[currentSong].List_DanSongs[0].Dan_C[0] != null)
						tSoulDraw(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Value_Gauge[0], OpenNijiiroRW.Skin.DaniSelect_Value_Gauge[1], stバー情報[currentSong].List_DanSongs[0].Dan_C[0].GetValue()[0]);

					//TJAPlayer3.stage選曲.act曲リスト.ResolveTitleTexture(this.ttkExams[0]).t2D下中央基準描画((int)(scroll + 396 + Anime), 452);
					TitleTextureKey.ResolveTitleTexture(this.ttkExams[0]).t2D拡大率考慮中央基準描画((int)(scroll + Anime) + OpenNijiiroRW.Skin.DaniSelect_Text_Gauge[0], OpenNijiiroRW.Skin.DaniSelect_Text_Gauge[1]);
					...
					for (int i = 0; i < stバー情報[currentSong].ttkタイトル.Length - 1; i++)
					{
						...
						TitleTextureKey.ResolveTitleTexture(stバー情報[currentSong].ttkタイトル[i]).t2D描画(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Title_X[pos], OpenNijiiroRW.Skin.DaniSelect_Title_Y[pos]);
					...
					for (int j = 1; j < CExamInfo.cMaxExam; j++)  //段位条件のループ(魂ゲージを除く) 縦(y)
					{
						...
						for (int i = 0; i < stバー情報[currentSong].List_DanSongs.Count; i++)  //曲ごとのループ(魂ゲージを除く) 横(x)
						{
							if (stバー情報[currentSong].List_DanSongs[i].Dan_C[j] != null)
							{
								...
								if (stバー情報[currentSong].List_DanSongs[stバー情報[currentSong].List_DanSongs.Count - 1].Dan_C[j] != null)
								{
									...
									int moveY = OpenNijiiroRW.Skin.DaniSelect_Exam_Interval[1];
									int x = OpenNijiiroRW.Skin.DaniSelect_Exam_Bloc_X[index];
									int y = OpenNijiiroRW.Skin.DaniSelect_Exam_Bloc_Y[index];

									int exam_x = OpenNijiiroRW.Skin.DaniSelect_Exam_X[index];
									...
									switch (stバー情報[currentSong].List_DanSongs.Count)
									{
										...
										case 2:
										case 3:
											tex = OpenNijiiroRW.Tx.Dani_Bloc[1];
											break;
										case 4:
										case 5:
										case 6:
										default:
											tex = OpenNijiiroRW.Tx.Dani_Bloc[3];
											moveX /= 2;
											moveY /= 2;
											exam_x = OpenNijiiroRW.Skin.DaniSelect_Exam_X_Ex[index];
											exam_y = OpenNijiiroRW.Skin.DaniSelect_Exam_Y_Ex[index];
											break;
									}
								...
								else
								{
									...
									if (i == 0)
									{
										...
										OpenNijiiroRW.Tx.Dani_Bloc[0]?.t2D描画(
											scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Exam_Bloc_X[index],
											OpenNijiiroRW.Skin.DaniSelect_Exam_Bloc_Y[index]);
									}

									tExamDraw(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_Exam_X[index], OpenNijiiroRW.Skin.DaniSelect_Exam_Y[index], stバー情報[currentSong].List_DanSongs[0].Dan_C[j].GetValue()[0], stバー情報[currentSong].List_DanSongs[0].Dan_C[j].ExamRange);
						...
						if (stバー情報[currentSong].List_DanSongs[0].Dan_C[j] != null)
						{
							...
							tmpTex.t2D拡大率考慮中央基準描画((int)(scroll + Anime) + OpenNijiiroRW.Skin.DaniSelect_Exam_Title_X[index], OpenNijiiroRW.Skin.DaniSelect_Exam_Title_Y[index]);
						}
			...
			case CSongListNode.ENodeType.BOX:
				{
					...
					TitleTextureKey.ResolveTitleTexture(stバー情報[currentSong].ttkタイトル[0])
						.t2D拡大率考慮上中央基準描画((int)(scroll + Anime + OpenNijiiroRW.Skin.DaniSelect_FolderText_X[0]), OpenNijiiroRW.Skin.DaniSelect_FolderText_Y[0]);
				...
				case CSongListNode.ENodeType.SCORE:
					{
						...
						var TableEntry = OpenNijiiroRW.SaveFileInstances[OpenNijiiroRW.SaveFile].data.tGetSongSelectTableEntry(song.tGetUniqueId());
				...
				case CSongListNode.ENodeType.BOX:
					{
						...
						stバー情報[i].ttkタイトル[0] = new TitleTextureKey(song.ldTitle.GetString(""), pfDanFolder, Color.White, Color.Black, OpenNijiiroRW.Skin.Resolution[0]);
						for (int boxdesc = 0; boxdesc < 3; boxdesc++)
							if (song.strBoxText[boxdesc] != null)
								stバー情報[i].ttkタイトル[boxdesc + 1] = new TitleTextureKey(song.strBoxText[boxdesc].GetString(""), pfDanFolderDesc, song.ForeColor, song.BackColor, OpenNijiiroRW.Skin.Resolution[0]);
							else
								stバー情報[i].ttkタイトル[boxdesc + 1] = new TitleTextureKey("", pfDanFolderDesc, Color.White, Color.Black, OpenNijiiroRW.Skin.Resolution[0]);
	...
	public void tLevelNumberDraw(float x, float y, int num, float scale = 1.0f)
	{
		...
		float width = OpenNijiiroRW.Tx.Dani_Level_Number.sz画像サイズ.Width / 10.0f;
	...
	public void tSoulDraw(float x, float y, int num)
	{
		...
		float width = OpenNijiiroRW.Tx.Dani_Soul_Number.sz画像サイズ.Width / 10.0f;
		float height = OpenNijiiroRW.Tx.Dani_Soul_Number.sz画像サイズ.Height / 2.0f;

		float text_width = OpenNijiiroRW.Skin.DaniSelect_Soul_Number_Text_Width;

		OpenNijiiroRW.Tx.Dani_Soul_Number.t2D描画(x + OpenNijiiroRW.Skin.DaniSelect_Soul_Number_Interval[0] + (width / 2),
		...
		for (int j = 0; j < nums.Length; j++)
		{
			...
			float _y = y - (OpenNijiiroRW.Skin.DaniSelect_Soul_Number_Interval[1] * offset) - (height / 2);

			OpenNijiiroRW.Tx.Dani_Soul_Number.t2D描画(_x, _y,
				new RectangleF(width * nums[j], 0, width, height));
		}
	}
	...
	public void tExamDraw(float x, float y, int num, Exam.Range Range, float scale = 1.0f)
	{
        ...
        */
		...
		float width = OpenNijiiroRW.Tx.Dani_Exam_Number.sz画像サイズ.Width / 10.0f;
		float height = OpenNijiiroRW.Tx.Dani_Exam_Number.sz画像サイズ.Height / 2.0f;
		...
		for (int j = 0; j < nums.Length; j++)
		{
			...
			float _y = y - (((OpenNijiiroRW.Skin.DaniSelect_Exam_Number_Interval[1] * offset) - (height / 2)) * scale);

			OpenNijiiroRW.Tx.Dani_Exam_Number.t2D描画(_x, _y,
				new RectangleF(width * nums[j], 0, width, height));
		}
	}
