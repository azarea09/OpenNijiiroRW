...
internal class CActConfigKeyAssign : CActivity
{
	...
	public override int Draw()
	{
		if (!base.IsDeActivated)
		{
			...
			int num5 = OpenNijiiroRW.Skin.Config_KeyAssign_Move;
			int x = OpenNijiiroRW.Skin.Config_KeyAssign_Font[0];
			int y = OpenNijiiroRW.Skin.Config_KeyAssign_Font[1];
			...
			for (int i = 0; i < 0x10; i++)
			{
				switch (stkeyassignArray[i].InputDevice)
				{
					...
					default:
						OpenNijiiroRW.stageConfig.actFont.t文字列描画(x + num5, y, string.Format("{0,2}.", i + 1), this.n現在の選択行 == i, 0.75f);
						break;
				}
				y += num5;
			}
			OpenNijiiroRW.stageConfig.actFont.t文字列描画(x + num5, y, "Reset", this.n現在の選択行 == 0x10, 0.75f);
			y += num5;
			...
			if (device.CurrentType == InputDeviceType.Gamepad)
			{
				for (int i = 0; i < device.ButtonStates.Length; i++)
				{
					if (device.KeyPressed(i))
					{
						...
						OpenNijiiroRW.ConfigIni.KeyAssign[(int)this.part][(int)this.pad][this.n現在の選択行].ID = device.ID;
	...
	private bool tキーチェックとアサイン_Keyboard()
	{
		for (int i = 0; i < 144; i++)
		{
				...
				OpenNijiiroRW.InputManager.Keyboard.KeyPressed(i))
			{
				...
				OpenNijiiroRW.ConfigIni.KeyAssign[(int)this.part][(int)this.pad][this.n現在の選択行].InputDevice = EInputDevice.Keyboard;
				OpenNijiiroRW.ConfigIni.KeyAssign[(int)this.part][(int)this.pad][this.n現在の選択行].ID = 0;
				OpenNijiiroRW.ConfigIni.KeyAssign[(int)this.part][(int)this.pad][this.n現在の選択行].Code = i;
				return true;
			}
