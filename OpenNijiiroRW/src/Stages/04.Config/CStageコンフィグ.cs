...
internal class CStageコンフィグ : CStage {
	...
	public override void CreateManagedResource()                                            // OPTIONと画像以外共通
	{
		...
		//}
		this.ftフォント = HPrivateFastFont.tInstantiateMainFont((int)OpenNijiiroRW.Skin.Config_Font_Scale_Description, CFontRenderer.FontStyle.Bold);


		OpenNijiiroRW.Tx.Config_Cursor = OpenNijiiroRW.tテクスチャの生成(CSkin.Path($"{TextureLoader.BASE}{TextureLoader.CONFIG}Cursor.png"));
		...
		*/
		...
		if (OpenNijiiroRW.Tx.Config_Cursor != null) {
			...
			*/
			...
			int x = OpenNijiiroRW.Skin.Config_Item_X[this.n現在のメニュー番号];
			int y = OpenNijiiroRW.Skin.Config_Item_Y[this.n現在のメニュー番号];

			int width = OpenNijiiroRW.Tx.Config_Cursor.sz画像サイズ.Width / 3;
			int height = OpenNijiiroRW.Tx.Config_Cursor.sz画像サイズ.Height;

			int move = OpenNijiiroRW.Skin.Config_Item_Width;

			//Left
			OpenNijiiroRW.Tx.Config_Cursor.t2D中心基準描画(x - (width / 2) - move, y,
		...
		if (this.tx説明文パネル != null)
			this.tx説明文パネル.t2D描画(OpenNijiiroRW.Skin.Config_ExplanationPanel[0], OpenNijiiroRW.Skin.Config_ExplanationPanel[1]);
		//---------------------
		#endregion
