...
class CNamePlate {
	...
	public CNamePlate() {
		for (int player = 0; player < 5; player++) {
			...
			if (OpenNijiiroRW.SaveFileInstances[player].data.TitleType < 0) OpenNijiiroRW.SaveFileInstances[player].data.TitleType = 0;
	...
	public void tNamePlateRefreshTitles(int player) {
		...
		if (OpenNijiiroRW.SaveFileInstances[player].data.DanGold)
			lcNamePlate.SetInfos(player, name, title, $"<g.#FFE34A.#EA9622>{dan}</g>", OpenNijiiroRW.SaveFileInstances[actualPlayer].data);
		else
			lcNamePlate.SetInfos(player, name, title, dan, OpenNijiiroRW.SaveFileInstances[actualPlayer].data);
	}
	...
	public void tNamePlateDraw(int x, int y, int player, bool bTitle = false, int Opacity = 255) {
		...
		lcNamePlate.Draw(x, y, Opacity, basePlayer, OpenNijiiroRW.P1IsBlue() ? 1 : 0);
	}
