...
class CStageOnlineLounge : CStage
{
	...
	public override void Activate()
	{
		...
		OpenNijiiroRW.Skin.soundOnlineLoungeBGM?.tPlay();
		...
		dbCDN = OpenNijiiroRW.Databases.DBCDN;
		dbCDNData = null;
	...
	public override void DeActivate()
	{
		...
		OpenNijiiroRW.Songs管理.UpdateDownloadBox();
	...
	public override void CreateManagedResource()
	{
		...
		this.pfOLFontLarge = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.OnlineLounge_Font_OLFontLarge);
	...
	public override int Draw()
	{
			...
			|| currentMenu == ECurrentMenu.CDN_OPTION)
		{
			...
			for (int i = 0; i < _ref.Length; i++)
			{
				...
				OpenNijiiroRW.Tx.OnlineLounge_Side_Menu?.t2D拡大率考慮上中央基準描画(baseX + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Move[0] * i,
					baseY + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Move[1] * i);
				tmpTex.t2D拡大率考慮上中央基準描画(
					baseX + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Text_Offset[0] + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Move[0] * i,
					baseY + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Text_Offset[1] + OpenNijiiroRW.Skin.OnlineLounge_Side_Menu_Move[1] * i);
			}
		}
				...
				if (pos != 0 && i == 0)
				{
					...
					for (int k = 0; k < (int)Difficulty.Total; k++)
					{
						...
						if (diff > 0)
						{
							...
							difnb_?.t2D中心基準描画(OpenNijiiroRW.Skin.OnlineLounge_Context_Level[0] + column, OpenNijiiroRW.Skin.OnlineLounge_Context_Level[1] + row);
						}
		...
		if (IsDownloading)
		{
			...
			text.t2D中心基準描画(OpenNijiiroRW.Skin.OnlineLounge_Downloading[0], OpenNijiiroRW.Skin.OnlineLounge_Downloading[1]);
		}
			...
			if (downloadBox != null)
			{
				...
				if (!Directory.Exists(genredPath))
				{
					...
					if (correspondingBox == null)
					{
						...
						StreamWriter sw = new StreamWriter(newBoxDef, false, Encoding.GetEncoding(OpenNijiiroRW.sEncType));
