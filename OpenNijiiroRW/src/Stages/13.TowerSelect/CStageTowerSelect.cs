...
class CStageTowerSelect : CStage {
	...
	public override void Activate() {
		...
		if (listSongs == null)
			listSongs = OpenNijiiroRW.Songs管理.list曲ルート_Tower;
	...
	public override void CreateManagedResource() {
		...
		pfSubTitleFont = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.TowerSelect_SubTitle_Size);
	...
	public override int Draw() {
		...
		for (int i = 0; i < OpenNijiiroRW.Skin.TowerSelect_Bar_Count; i++) {
			...
			int x = OpenNijiiroRW.Skin.TowerSelect_Bar_X[i];
			int y = OpenNijiiroRW.Skin.TowerSelect_Bar_Y[i];
			tDrawTower(x, y, bar);
		}
	...
	private bool tSelectSongRandomly() {
		...
		OpenNijiiroRW.stageSongSelect.t指定された曲の子リストの曲を列挙する_孫リスト含む(song.rParentNode, ref songs, ref mandatoryDiffs, true, Difficulty.Tower);
	...
	private void tDrawTower(int x, int y, BarInfo barInfo) {
		...
		TitleTextureKey.ResolveTitleTexture(barInfo.ttkTitle).t2D拡大率考慮中央基準描画(x + OpenNijiiroRW.Skin.TowerSelect_Title_Offset[0], y + OpenNijiiroRW.Skin.TowerSelect_Title_Offset[1]);
	...
	private void tSetBarInfos() {
		for (int i = 0; i < BarInfos.Length; i++) {
			...
			bar.ttkTitle = new TitleTextureKey(bar.strTitle, pfTitleFont, Color.Black, Color.Transparent, OpenNijiiroRW.Skin.TowerSelect_Title_MaxWidth);
			bar.ttkSubTitle = new TitleTextureKey(bar.strSubTitle, pfTitleFont, Color.Black, Color.Transparent, OpenNijiiroRW.Skin.TowerSelect_SubTitle_MaxWidth);
		}
	}
