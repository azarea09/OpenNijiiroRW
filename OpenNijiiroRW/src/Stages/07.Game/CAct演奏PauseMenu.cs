...
internal class CAct演奏PauseMenu : CActSelectPopupMenu
{
	...
	private List<CItemBase> MakeListCItemBase(int nConfigSet, int nInst)
	{
		...
		if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] != (int)Difficulty.Dan) l.Add(new CSwitchItemList(CLangManager.LangInstance.GetString("PAUSE_RESTART"), CItemBase.EPanelType.Normal, 0, "", "", new string[] { "" }));
	...
	public override void tEnter押下Main(int nSortOrder)
	{
		switch (n現在の選択行)
		{
			case (int)EOrder.Continue:
				...
				OpenNijiiroRW.TJA.t全チップの再生再開();
				OpenNijiiroRW.stageGameScreen.actAVI.Resume();
