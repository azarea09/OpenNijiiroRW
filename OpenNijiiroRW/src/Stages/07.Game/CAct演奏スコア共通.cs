...
internal class CAct演奏スコア共通 : CActivity {
	...
	public void Add(long delta, int player) {
		...
		delta = (long)(delta * OpenNijiiroRW.stageSongSelect.actPlayOption.tGetModMultiplier(CActPlayOption.EBalancingType.SCORE, false, player));
		...
		for (int sc = 0; sc < 1; sc++) {
			for (int i = 0; i < 256; i++) {
				if (this.stScore[i].b使用中 == false) {
					...
					this.stScore[i].ctTimer = new CCounter(0, 465, 2, OpenNijiiroRW.Timer);
		...
		for (int sc = 0; sc < 1; sc++) {
			for (int i = 0; i < 256; i++) {
				if (this.stScore[i].b使用中 == false) {
					...
					this.stScore[i].ctTimer = new CCounter(0, 100, 4, OpenNijiiroRW.Timer);
	...
	protected void t小文字表示(int x, int y, string str, int mode, int alpha, int player) {
		foreach (char ch in str) {
			...
			x += OpenNijiiroRW.Skin.Game_Score_Padding;
		}
	}
}
