...
internal class CAct演奏パネル文字列 : CActivity
{
	...
	private readonly Dictionary<string, Color4> tTagDict = new Dictionary<string, Color4>
	{
		...
		["OpenNijiiroRW Original"] = new Color4(tToArgb(249, 255, 40)),
	...
	public void SetPanelString(string songName, string genreName, string stageText = null, CSongListNode songNode = null)
	{
		if (base.IsActivated)
		{
			...
			this.txGENRE = OpenNijiiroRW.Tx.TxCGen("Template");
			...
			pfGENRE = HPrivateFastFont.tInstantiateBoxFont(OpenNijiiroRW.Skin.Game_GenreText_FontSize);

			this.ttkGENRE = new TitleTextureKey(genreName, this.pfGENRE, Color.White, Color.Black, 1000);

			this.ct進行用 = new CCounter(0, 2000, 2, OpenNijiiroRW.Timer);
			this.Start();
	...
	public override void DeActivate()
	{
		...
		OpenNijiiroRW.tDisposeSafely(ref this.txGENRE);
		OpenNijiiroRW.tDisposeSafely(ref this.pfGENRE);
		OpenNijiiroRW.tDisposeSafely(ref this.txPanel);
