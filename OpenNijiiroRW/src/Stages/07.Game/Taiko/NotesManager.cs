...
class NotesManager {
	...
	public static int GetNoteY(double msDTime, double th16DBeat, double bpm, double scroll, EScrollMode eScrollMode) {
		...
		int pxPer4Beats = OpenNijiiroRW.Skin.Game_Notes_Interval;
		double screenScale = OpenNijiiroRW.Skin.Resolution[1] / 720.0;
		double n4Beats = getN4Beats(msDTime, th16DBeat, bpm, eScrollMode);
		return (int)(n4Beats * pxPer4Beats * scroll * screenScale);
	}
	...
	public static void DisplayNote(int player, int x, int y, int Lane) {
		...
		switch (Lane) {
			...
			case 3:
			case 4:
				OpenNijiiroRW.Tx.Notes[(int)_gt]?.t2D中心基準描画(x, y, new Rectangle(Lane * OpenNijiiroRW.Skin.Game_Notes_Size[0], OpenNijiiroRW.Skin.Game_Notes_Size[1] * 3, OpenNijiiroRW.Skin.Game_Notes_Size[0], OpenNijiiroRW.Skin.Game_Notes_Size[1]));
				break;
			case 5:
				OpenNijiiroRW.Tx.Note_Swap?.t2D中心基準描画(x, y, new Rectangle(0, OpenNijiiroRW.Skin.Game_Notes_Size[1] * 3, OpenNijiiroRW.Skin.Game_Notes_Size[0], OpenNijiiroRW.Skin.Game_Notes_Size[1]));
				break;
		}
	}
	...
	public static void DisplayRoll(int player, int x, int y, CChip chip, int frame,
		...
		int _offset = 0;
		var _texarr = OpenNijiiroRW.Tx.Notes[(int)_gt];
		int rollOrigin = (OpenNijiiroRW.Skin.Game_Notes_Size[0] * 5);
		float wImage = OpenNijiiroRW.Skin.Game_Notes_Size[0];
		float hImage = OpenNijiiroRW.Skin.Game_Notes_Size[1];
		...
		if (chip.bShowRoll) {
			...
			if (OpenNijiiroRW.Skin.Game_RollColorMode != CSkin.RollColorMode.None)
			...
			_texarr.t2D_DisplayImage_RollNote((int)_center_x, (int)_center_y, new Rectangle(
				rollOrigin + OpenNijiiroRW.Skin.Game_Notes_Size[0] + _offset,
				0,
			...
			_texarr.t2D描画((int)(_xc - xTailOrig), (int)(_yc - yTailOrig), 0, new Rectangle(
				rollOrigin + (OpenNijiiroRW.Skin.Game_Notes_Size[0] * 2) + _offset,
				frame,
				OpenNijiiroRW.Skin.Game_Notes_Size[0],
				OpenNijiiroRW.Skin.Game_Notes_Size[1]));
		...
		// Head
		_texarr.t2D描画(x, y, 0, new Rectangle(rollOrigin + _offset, frame, OpenNijiiroRW.Skin.Game_Notes_Size[0], OpenNijiiroRW.Skin.Game_Notes_Size[1]));
		_texarr.color4 = normalColor;
	}
