...
internal class CActImplRoll : CActivity {
		...
		for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++) {
			...
			if (this.ct連打枠カウンター[player].IsUnEnded) {
				...
				if (ct連打枠カウンター[player].CurrentValue == 0 || ct連打枠カウンター[player].CurrentValue == 60) {
					bNowRollAnime = 0;
					OpenNijiiroRW.Tx.Balloon_Number_Roll.Opacity = 64;
				} else if (ct連打枠カウンター[player].CurrentValue == 1 || ct連打枠カウンター[player].CurrentValue == 59) {
					bNowRollAnime = 1;
					OpenNijiiroRW.Tx.Balloon_Number_Roll.Opacity = 128;
				} else if (ct連打枠カウンター[player].CurrentValue == 2 || ct連打枠カウンター[player].CurrentValue == 58) {
					bNowRollAnime = 2;
					OpenNijiiroRW.Tx.Balloon_Number_Roll.Opacity = 192;
				} else if (ct連打枠カウンター[player].CurrentValue == 3 || ct連打枠カウンター[player].CurrentValue == 57) {
					bNowRollAnime = 3;
					OpenNijiiroRW.Tx.Balloon_Number_Roll.Opacity = 255;
				} else if (ct連打枠カウンター[player].CurrentValue >= 4 || ct連打枠カウンター[player].CurrentValue <= 56) {
					bNowRollAnime = 4;
					OpenNijiiroRW.Tx.Balloon_Number_Roll.Opacity = 255;
				}

				float width = OpenNijiiroRW.Tx.Balloon_Roll.szTextureSize.Width / 5.0f;
				float height = OpenNijiiroRW.Tx.Balloon_Roll.szTextureSize.Height;

				OpenNijiiroRW.Tx.Balloon_Roll?.t2D描画(OpenNijiiroRW.Skin.Game_Balloon_Roll_Frame_X[player], OpenNijiiroRW.Skin.Game_Balloon_Roll_Frame_Y[player], new RectangleF(0 + bNowRollAnime * width, 0, width, height));
				this.t文字表示(OpenNijiiroRW.Skin.Game_Balloon_Roll_Number_X[player], OpenNijiiroRW.Skin.Game_Balloon_Roll_Number_Y[player], n連打数, player);
			}
		}
	...
	public void t枠表示時間延長(int player, bool first) {
		if ((this.ct連打枠カウンター[player].CurrentValue >= 6 && !first) || first)
			this.ct連打枠カウンター[player] = new CCounter(0, 60, 40, OpenNijiiroRW.Timer);
	...
	private void t文字表示(int x, int y, int num, int nPlayer) {
		...
		for (int j = 0; j < nums.Length; j++) {
			...
			float width = OpenNijiiroRW.Tx.Balloon_Number_Roll.sz画像サイズ.Width / 10.0f;
			float height = OpenNijiiroRW.Tx.Balloon_Number_Roll.sz画像サイズ.Height;

			OpenNijiiroRW.Tx.Balloon_Number_Roll.t2D拡大率考慮下基準描画(_x, _y, new RectangleF(width * nums[j], 0, width, height));
		}
	}
}
