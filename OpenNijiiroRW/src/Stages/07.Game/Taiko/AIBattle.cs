...
class AIBattle : CStage {
	...
	public override int Draw() {
		...
		OpenNijiiroRW.Tx.AIBattle_SectionTime_Panel?.t2D描画(OpenNijiiroRW.Skin.Game_AIBattle_SectionTime_Panel[0], OpenNijiiroRW.Skin.Game_AIBattle_SectionTime_Panel[1]);
		...
		var nowSection = OpenNijiiroRW.stageGameScreen.NowAIBattleSection;
		...
		for (int i = 0; i < OpenNijiiroRW.stageGameScreen.AIBattleSections.Count; i++) {
			...
			int base_height = OpenNijiiroRW.Tx.AIBattle_Batch_Base.szTextureSize.Height;

			int base_x = OpenNijiiroRW.Skin.Game_AIBattle_Batch_Base[0] + (OpenNijiiroRW.Skin.Game_AIBattle_Batch_Move[0] * i);
			int base_y = OpenNijiiroRW.Skin.Game_AIBattle_Batch_Base[1] + (OpenNijiiroRW.Skin.Game_AIBattle_Batch_Move[1] * upDown);
		...
		for (int i = 0; i < OpenNijiiroRW.stageGameScreen.NowAIBattleSectionCount; i++) {
			...
			int width = OpenNijiiroRW.Tx.AIBattle_Batch.szTextureSize.Width / 6;
			int height = OpenNijiiroRW.Tx.AIBattle_Batch.szTextureSize.Height / 2;
			...
			float _x = OpenNijiiroRW.Skin.Game_AIBattle_Batch[0] + (OpenNijiiroRW.Skin.Game_AIBattle_Batch_Move[0] * i);
			float _y = OpenNijiiroRW.Skin.Game_AIBattle_Batch[1] + (OpenNijiiroRW.Skin.Game_AIBattle_Batch_Move[1] * upDown);

			_x = OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime[0] + ((_x - OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime[0]) * value);
			_y = OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime[1] + ((_y - OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime[1]) * value);


			float size_x = OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime_Size[0] +
						   ((OpenNijiiroRW.Skin.Game_AIBattle_Batch_Size[0] - OpenNijiiroRW.Skin.Game_AIBattle_Batch_Anime_Size[0]) * value);
			...
			switch (section.End) {
				...
				case CStage演奏画面共通.AIBattleSection.EndType.Lose:
					OpenNijiiroRW.Tx.AIBattle_Batch?.t2D拡大率考慮中央基準描画(_x, _y, new System.Drawing.RectangleF(width * drawFrame, height, width, height));
					break;
			}
		}
		...
		for (int player = 0; player < 2; player++) {
			...
			{
				OpenNijiiroRW.stageGameScreen.CSectionScore[player].nGreat,
				OpenNijiiroRW.stageGameScreen.CSectionScore[player].nGood,
				OpenNijiiroRW.stageGameScreen.CSectionScore[player].nMiss,
				OpenNijiiroRW.stageGameScreen.CSectionScore[player].nRoll
			};

			int[] num_x = new int[4]
			{
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Perfect_X[player],
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Good_X[player],
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Miss_X[player],
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Roll_X[player]
			};

			int[] num_y = new int[4]
			{
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Perfect_Y[player],
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Good_Y[player],
				OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Miss_Y[player],
	...
	private void DrawJudgeNumber(int x, int y, int num) {
		...
		for (int j = 0; j < nums.Length; j++) {
			...
			float height = OpenNijiiroRW.Tx.AIBattle_Judge_Number.sz画像サイズ.Height;

			float _x = x - (OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Interval[0] * offset);
			float _y = y - (OpenNijiiroRW.Skin.Game_AIBattle_Judge_Number_Interval[1] * offset);

			OpenNijiiroRW.Tx.AIBattle_Judge_Number.t2D拡大率考慮中央基準描画(_x + (width / 2), _y + (height / 2),
