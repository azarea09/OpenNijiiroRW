...
internal class CActImplClearAnimation : CActivity {
	...
	public void Start() {
		...
		this.ct進行メイン = new CCounter(0, 300, 22, OpenNijiiroRW.Timer);
	...
	public override void Activate() {
		...
		if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Tower) {
			...
			this.soundDanFailed = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Dan_Fail.ogg"), ESoundGroup.SoundEffect);
			this.soundDanRedClear = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Dan_Red_Pass.ogg"), ESoundGroup.SoundEffect);
			this.soundDanRedFC = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Dan_Red_FullCombo.ogg"), ESoundGroup.SoundEffect);
			this.soundDanRedPerfect = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Dan_Red_Perfect.ogg"), ESoundGroup.SoundEffect);
			this.soundDanGoldClear = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Dan_Gold_Pass.ogg"), ESoundGroup.SoundEffect);
			...
			this.soundAILose = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}AIBattle_Lose.ogg"), ESoundGroup.SoundEffect);
	...
	public override void CreateManagedResource() {
		...
		for (int i = 0; i < OpenNijiiroRW.MAX_PLAYERS; i++) {
			...
			this.soundPerfectCombo[i] = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}AllPerfect.ogg"), ESoundGroup.SoundEffect);
		}
		...
		if (this.ct進行メイン != null && (OpenNijiiroRW.stageGameScreen.ePhaseID == CStage.EPhase.Game_EndStage || OpenNijiiroRW.stageGameScreen.ePhaseID == CStage.EPhase.Game_STAGE_FAILED || OpenNijiiroRW.stageGameScreen.ePhaseID == CStage.EPhase.Game_STAGE_CLEAR_FadeOut)) {
				...
				for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++) {
					...
					switch (this.Mode[i]) {
						case EndMode.StageFailed:
							...
							OpenNijiiroRW.Skin.voiceClearFailed[OpenNijiiroRW.GetActualPlayer(i)]?.SetPanning(pan);
							OpenNijiiroRW.Skin.voiceClearFailed[OpenNijiiroRW.GetActualPlayer(i)]?.tPlay();
							break;
						case EndMode.StageCleared:
							ClearScript.PlayEndAnime(i);
							this.soundClear[i]?.SetPanning(pan);
							this.soundClear[i]?.PlayStart();
							OpenNijiiroRW.Skin.voiceClearClear[OpenNijiiroRW.GetActualPlayer(i)]?.SetPanning(pan);
							OpenNijiiroRW.Skin.voiceClearClear[OpenNijiiroRW.GetActualPlayer(i)]?.tPlay();
							break;
						case EndMode.StageFullCombo:
							...
							OpenNijiiroRW.Skin.voiceClearFullCombo[OpenNijiiroRW.GetActualPlayer(i)]?.SetPanning(pan);
							OpenNijiiroRW.Skin.voiceClearFullCombo[OpenNijiiroRW.GetActualPlayer(i)]?.tPlay();
							break;
						case EndMode.StagePerfectCombo:
							...
							OpenNijiiroRW.Skin.voiceClearAllPerfect[OpenNijiiroRW.GetActualPlayer(i)]?.SetPanning(pan);
							OpenNijiiroRW.Skin.voiceClearAllPerfect[OpenNijiiroRW.GetActualPlayer(i)]?.tPlay();
							break;

						case EndMode.AI_Lose:
							AILoseScript.PlayEndAnime(i);
							this.soundAILose?.PlayStart();
							OpenNijiiroRW.Skin.voiceAILose[OpenNijiiroRW.GetActualPlayer(1)]?.tPlay();
							break;
						case EndMode.AI_Win:
							AIWinScript.PlayEndAnime(i);
							this.soundAIWin?.PlayStart();
							OpenNijiiroRW.Skin.voiceAIWin[OpenNijiiroRW.GetActualPlayer(i)]?.tPlay();
							break;
			...
			for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++) {
				...
				switch (this.Mode[i]) {
					...
					case EndMode.AI_Win:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) AIWinScript.Update(i);
						AIWinScript.Draw(i);
						break;
					case EndMode.AI_Lose:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) AILoseScript.Update(i);
						AILoseScript.Draw(i);
						break;
					case EndMode.AI_Win_FullCombo:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) AIWin_FullComboScript.Update(i);
						AIWin_FullComboScript.Draw(i);
						break;
					case EndMode.AI_Win_Perfect:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) AIWin_PerfectScript.Update(i);
						AIWin_PerfectScript.Draw(i);
						break;

					case EndMode.Tower_Dropout:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Tower_DropoutScript.Update(i);
						Tower_DropoutScript.Draw(i);
						break;
					case EndMode.Tower_TopReached_Pass:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Tower_TopReached_PassScript.Update(i);
						Tower_TopReached_PassScript.Draw(i);
						break;
					case EndMode.Tower_TopReached_FullCombo:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Tower_TopReached_FullComboScript.Update(i);
						Tower_TopReached_FullComboScript.Draw(i);
						break;
					case EndMode.Tower_TopReached_Perfect:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Tower_TopReached_PerfectScript.Update(i);
						Tower_TopReached_PerfectScript.Draw(i);
						break;

					case EndMode.Dan_Fail:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_FailScript.Update(i);
						Dan_FailScript.Draw(i);
						break;
					case EndMode.Dan_Red_Pass:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Red_PassScript.Update(i);
						Dan_Red_PassScript.Draw(i);
						break;
					case EndMode.Dan_Red_FullCombo:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Red_FullComboScript.Update(i);
						Dan_Red_FullComboScript.Draw(i);
						break;
					case EndMode.Dan_Red_Perfect:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Red_PerfectScript.Update(i);
						Dan_Red_PerfectScript.Draw(i);
						break;
					case EndMode.Dan_Gold_Pass:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Gold_PassScript.Update(i);
						Dan_Gold_PassScript.Draw(i);
						break;
					case EndMode.Dan_Gold_FullCombo:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Gold_FullComboScript.Update(i);
						Dan_Gold_FullComboScript.Draw(i);
						break;
					case EndMode.Dan_Gold_Perfect:
						if (!OpenNijiiroRW.stageGameScreen.bPAUSE) Dan_Gold_PerfectScript.Update(i);
