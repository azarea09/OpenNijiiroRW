		...
		if (!base.IsDeActivated) {
			...
			for (int i = 0; i < OpenNijiiroRW.ConfigIni.nPlayerCount; i++) {
				if (OpenNijiiroRW.ConfigIni.nPlayerCount == 5) {
					...
					add_y[i] = OpenNijiiroRW.Skin.Game_Score_Add_Y[i];
					addBonus_x[i] = OpenNijiiroRW.Skin.Game_Score_AddBonus_X[i];
					addBonus_y[i] = OpenNijiiroRW.Skin.Game_Score_AddBonus_Y[i];
				}
			}
			...
			for (int i = 0; i < 256; i++) {
				if (this.stScore[i].b使用中) {
					if (!this.stScore[i].ctTimer.IsStoped) {
						...
						if (this.nNowDisplayedAddScore < 10 && this.stScore[i].bBonusScore == false && !OpenNijiiroRW.ConfigIni.SimpleMode)
