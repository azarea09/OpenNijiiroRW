...
internal class CActImplBackground : CActivity
{
	...
	public override void Activate()
	{
		...
		this.pfTowerText = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.Game_Tower_Font_TowerText);
        ...
        */
		...
		this.currentCharacter = Math.Max(0, Math.Min(OpenNijiiroRW.SaveFileInstances[0].data.Character, OpenNijiiroRW.Skin.Characters_Ptn - 1));

		float resolutionScaleX = OpenNijiiroRW.Skin.Resolution[0] / (float)OpenNijiiroRW.Skin.Characters_Resolution[currentCharacter][0];
		...
		this.ctStandingAnimation = new CCounter(0, 1000, (60000f / (float)CTja.TjaBeatSpeedToGameBeatSpeed(OpenNijiiroRW.stageGameScreen.actPlayInfo.dbBPM[0])) * OpenNijiiroRW.Skin.Characters_Beat_Tower_Standing[currentCharacter] / OpenNijiiroRW.Skin.Characters_Tower_Standing_Ptn[currentCharacter], OpenNijiiroRW.Timer);
	...
	public override void DeActivate()
	{
		...
		OpenNijiiroRW.tDisposeSafely(ref UpScript);
		OpenNijiiroRW.tDisposeSafely(ref DownScript);
	...
	public override int Draw()
	{
			...
			if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Tower)
			{
				...
				if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Tower)
				{
					...
					if (CFloorManagement.CurrentNumberOfLives > 0)
						CFloorManagement.LastRegisteredFloor = OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1;
					...
					for (int idx = len - 1; idx >= 0; idx--)
					{
						...
						OpenNijiiroRW.Tx.Taiko_Combo[0].t2D描画(OpenNijiiroRW.Skin.Game_Tower_Floor_Number[0] - ((digitLength - 8) * (len - idx) * 1.4f),
							OpenNijiiroRW.Skin.Game_Tower_Floor_Number[1],
							new Rectangle(digitLength * currentNum, 0,
								digitLength, OpenNijiiroRW.Tx.Taiko_Combo[0].szTextureSize.Height));
					}
					...
					OpenNijiiroRW.Tx.Gauge_Soul_Fire?.t2D描画(OpenNijiiroRW.Skin.Gauge_Soul_Fire_X_Tower, OpenNijiiroRW.Skin.Gauge_Soul_Fire_Y_Tower, new Rectangle(soulfire_width * (this.ct炎.CurrentValue), 0, soulfire_width, soulfire_height));
					OpenNijiiroRW.Tx.Gauge_Soul?.t2D描画(OpenNijiiroRW.Skin.Gauge_Soul_X_Tower, OpenNijiiroRW.Skin.Gauge_Soul_Y_Tower, new Rectangle(0, soul_height, OpenNijiiroRW.Tx.Gauge_Soul.szTextureSize.Width, soul_height));
					...
					for (int idx = 0; idx < len; idx++)
					{
						...
						OpenNijiiroRW.Tx.Taiko_Combo[0].t2D描画(OpenNijiiroRW.Skin.Game_Tower_Life_Number[0] + ((digitLength - 8) * (len - idx) * 1.1f),
							OpenNijiiroRW.Skin.Game_Tower_Life_Number[1],
							new Rectangle(digitLength * currentNum, 0,
								digitLength, OpenNijiiroRW.Tx.Taiko_Combo[0].szTextureSize.Height));
					}

					OpenNijiiroRW.Tx.Taiko_Combo[0].color4 = new Color4(1f, 1f, 1f, 1f);
		...
		if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Tower)
		{
			...
			float nextPositionMax140 = Math.Min((OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1) / (float)nightTime, 1f);

			if (bFloorChanged == true)
				ctSlideAnimation.Start(0, 1000, 120f / CTja.TjaBeatSpeedToGameBeatSpeed((float)OpenNijiiroRW.stageGameScreen.actPlayInfo.dbBPM[0]), OpenNijiiroRW.Timer);
			...
			int nextTowerBase = ((OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1) / 10) % OpenNijiiroRW.Skin.Game_Tower_Ptn_Base[currentTower];
			int towerBase = (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] / 10) % OpenNijiiroRW.Skin.Game_Tower_Ptn_Base[currentTower];

			int currentDeco = OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] % OpenNijiiroRW.Skin.Game_Tower_Ptn_Deco[currentTower];
			int nextDeco = (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1) % OpenNijiiroRW.Skin.Game_Tower_Ptn_Deco[currentTower];

			// Microfix for the first floor suddenly changing texture
			if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] == 0 && OpenNijiiroRW.Skin.Game_Tower_Ptn_Deco[currentTower] > 1)
				currentDeco++;
			if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] == 0 && OpenNijiiroRW.Skin.Game_Tower_Ptn_Base[currentTower] > 1)
				towerBase++;

			int widthChange = (int)(progressFactor * OpenNijiiroRW.Skin.Game_Tower_Floors_Move[0]);
			int heightChange = (int)(progressFactor * OpenNijiiroRW.Skin.Game_Tower_Floors_Move[1]);

			// Current trunk
			if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] < maxFloor)
				OpenNijiiroRW.Tx.Tower_Base[currentTower][towerBase]?.t2D下中央基準描画(
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[0] + widthChange,
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[1] + heightChange); // 316 + 360
			else
				OpenNijiiroRW.Tx.Tower_Top[currentTower]?.t2D下中央基準描画(
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[0] + widthChange,
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[1] + heightChange);

			// Current deco
			OpenNijiiroRW.Tx.Tower_Deco[currentTower][currentDeco]?.t2D下中央基準描画(
				OpenNijiiroRW.Skin.Game_Tower_Floors_Deco[0] + widthChange,
				OpenNijiiroRW.Skin.Game_Tower_Floors_Deco[1] + heightChange);

			int originY = OpenNijiiroRW.Skin.Game_Tower_Floors_Move[1] - heightChange;

			// Next trunk
			if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1 < maxFloor)
				OpenNijiiroRW.Tx.Tower_Base[currentTower][nextTowerBase]?.t2D下中央基準描画(
			...
			else if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1 == maxFloor)
			{
				OpenNijiiroRW.Tx.Tower_Top[currentTower]?.t2D下中央基準描画(
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[0] - OpenNijiiroRW.Skin.Game_Tower_Floors_Move[0] + widthChange,
					OpenNijiiroRW.Skin.Game_Tower_Floors_Body[1] - OpenNijiiroRW.Skin.Game_Tower_Floors_Move[1] + heightChange,
					new Rectangle(0, originY, OpenNijiiroRW.Tx.Tower_Top[currentTower].szTextureSize.Width, OpenNijiiroRW.Tx.Tower_Top[currentTower].szTextureSize.Height - originY));
			}

			// Next deco
			if (OpenNijiiroRW.stageGameScreen.actPlayInfo.NowMeasure[0] + 1 <= maxFloor)
				OpenNijiiroRW.Tx.Tower_Deco[currentTower][nextDeco]?.t2D下中央基準描画(
					OpenNijiiroRW.Skin.Game_Tower_Floors_Deco[0] - OpenNijiiroRW.Skin.Game_Tower_Floors_Move[0] + widthChange,
					OpenNijiiroRW.Skin.Game_Tower_Floors_Deco[1] - OpenNijiiroRW.Skin.Game_Tower_Floors_Move[1] + heightChange);
			...
			if (bFloorChanged == true)
			{
				...
				ctClimbingAnimation.Start(0, 1000, (120000f / floorBPM) / OpenNijiiroRW.Skin.Characters_Tower_Climbing_Ptn[currentCharacter], OpenNijiiroRW.Timer);
				ctRunningAnimation.Start(0, 1000, (60000f / floorBPM) / OpenNijiiroRW.Skin.Characters_Tower_Running_Ptn[currentCharacter], OpenNijiiroRW.Timer);
				ctStandTiredAnimation.Start(0, 1000, (60000f / floorBPM) * OpenNijiiroRW.Skin.Characters_Beat_Tower_Standing_Tired[currentCharacter] / OpenNijiiroRW.Skin.Characters_Tower_Standing_Tired_Ptn[currentCharacter], OpenNijiiroRW.Timer);
				ctClimbTiredAnimation.Start(0, 1000, (120000f / floorBPM) / OpenNijiiroRW.Skin.Characters_Tower_Climbing_Tired_Ptn[currentCharacter], OpenNijiiroRW.Timer);
				ctRunTiredAnimation.Start(0, 1000, (60000f / floorBPM) / OpenNijiiroRW.Skin.Characters_Tower_Running_Tired_Ptn[currentCharacter], OpenNijiiroRW.Timer);
			}
			...
			if (stageEnded && !TowerFinished && !isClimbing)
			{
				...
				ctFailAnimation.Start(0, 20000, (60000f / floorBPM) * OpenNijiiroRW.Skin.Characters_Beat_Tower_Fail[currentCharacter] / OpenNijiiroRW.Skin.Characters_Tower_Fail_Ptn[currentCharacter], OpenNijiiroRW.Timer);
				TowerFinished = true;
			}
			...
			else
			{
				...
				else if (stageEnded && OpenNijiiroRW.Skin.Characters_Tower_Clear_Ptn[currentCharacter] > 0 && CFloorManagement.CurrentNumberOfLives > 0)
				{
					...
					OpenNijiiroRW.Tx.Characters_Tower_Clear[currentCharacter][animChar]?.t2D拡大率考慮下中心基準描画(OpenNijiiroRW.Skin.Game_Tower_Don[0], OpenNijiiroRW.Skin.Game_Tower_Don[1]);
				}
