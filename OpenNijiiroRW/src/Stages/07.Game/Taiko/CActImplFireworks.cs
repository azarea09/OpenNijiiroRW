	...
	public virtual void Start(int nLane, ENoteJudge judge, int player) {
		for (int j = 0; j < 3 * 4; j++) {
			...
			//for( int n = 0; n < 1; n++ )
			{
				...
				this.st状態[j].ct進行 = new CCounter(0, 6, 25, OpenNijiiroRW.Timer);
				...
				switch (nLane) {
					...
					case 0x1A:
					case 0x1B:
						this.st状態_大[j].ct進行 = new CCounter(0, 9, 20, OpenNijiiroRW.Timer);
		...
		if (!base.IsDeActivated) {
						...
						if (OpenNijiiroRW.Tx.Effects_Hit_Explosion != null) {
							...
							switch (st状態[i].judge) {
								...
								case ENoteJudge.Auto:
									if (!OpenNijiiroRW.ConfigIni.SimpleMode) OpenNijiiroRW.Tx.Effects_Hit_Explosion.t2D描画(nX, nY, new Rectangle(this.st状態[i].ct進行.CurrentValue * nWidth, n, nWidth, nHeight));
									break;
								case ENoteJudge.Good:
									OpenNijiiroRW.Tx.Effects_Hit_Explosion.t2D描画(nX, nY, new Rectangle(this.st状態[i].ct進行.CurrentValue * nWidth, n + nHeight, nWidth, nHeight));
									break;
								case ENoteJudge.Mine:
									OpenNijiiroRW.Tx.Effects_Hit_Bomb?.t2D描画(nX, nY, new Rectangle(this.st状態[i].ct進行.CurrentValue * nBombWidth, 0, nBombWidth, nBombHeight));
									break;
						...
						switch (st状態_大[i].judge) {
							...
							case ENoteJudge.Auto:
								if (this.st状態_大[i].nIsBig == 1 && !OpenNijiiroRW.ConfigIni.SimpleMode) {
                                    ...
                                    mat *= Matrix.Translation( TJAPlayer3.Skin.nScrollFieldX[0] - SampleFramework.RenderSurfaceSize.Width / 2.0f, -(TJAPlayer3.Skin.nJudgePointY[ this.st状態[ i ].nPlayer ] - SampleFramework.RenderSurfaceSize.Height / 2.0f), 0f );
                                    ...
                                    */
									...
									x += OpenNijiiroRW.stageGameScreen.GetJPOSCROLLX(this.st状態[i].nPlayer);
									y += OpenNijiiroRW.stageGameScreen.GetJPOSCROLLY(this.st状態[i].nPlayer);
			...
			for (int i = 0; i < 45; i++) {
				...
				if (this.st大音符花火[i].b使用中) {
                    ...
                    mat *= Matrix.Translation(this.st大音符花火[i].fX - SampleFramework.RenderSurfaceSize.Width / 2, -(this.st大音符花火[i].fY - SampleFramework.RenderSurfaceSize.Height / 2), 0f);
                    */
			...
			for (int i = 0; i < 256; i++) {
				if (this.st紙吹雪[i].b使用中) {
                    ...
                    mat *= Matrix.Translation(this.st紙吹雪[i].fX - SampleFramework.RenderSurfaceSize.Width / 2, -(this.st紙吹雪[i].fY - SampleFramework.RenderSurfaceSize.Height / 2), 0f);
                    */
