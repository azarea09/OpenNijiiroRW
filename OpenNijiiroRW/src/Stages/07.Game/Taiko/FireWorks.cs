	...
	public virtual void Start(int nLane, int nPlayer, double x, double y) {
		...
		for (int i = 0; i < 32; i++) {
			if (!FireWork[i].IsUsing) {
				...
				FireWork[i].X = x;
				FireWork[i].Y = y;
				FireWork[i].Counter = new CCounter(0, OpenNijiiroRW.Skin.Game_Effect_FireWorks[2] - 1, OpenNijiiroRW.Skin.Game_Effect_FireWorks_Timer, OpenNijiiroRW.Timer);
				break;
			}
		}
	}
		...
		if (!base.IsDeActivated && !OpenNijiiroRW.ConfigIni.SimpleMode) {
			for (int i = 0; i < 32; i++) {
				if (FireWork[i].IsUsing) {
					...
					OpenNijiiroRW.Tx.Effects_Hit_FireWorks?.t2D中心基準描画((float)FireWork[i].X, (float)FireWork[i].Y, 1, new Rectangle(FireWork[i].Counter.CurrentValue * OpenNijiiroRW.Skin.Game_Effect_FireWorks[0], 0, OpenNijiiroRW.Skin.Game_Effect_FireWorks[0], OpenNijiiroRW.Skin.Game_Effect_FireWorks[1]));
