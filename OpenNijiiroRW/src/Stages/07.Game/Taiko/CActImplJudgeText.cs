...
internal class CActImplJudgeText : CActivity
{
			...
			for (int j = 0; j < 5; j++)
			{
				for (int i = 0; i < JudgeAnimes[j].Count; i++)
				{
					...
					if (OpenNijiiroRW.Tx.Judge != null)
					{
						...
						x += (moveValue * OpenNijiiroRW.Skin.Game_Judge_Move[0]) + OpenNijiiroRW.stageGameScreen.GetJPOSCROLLX(j);
						y += (moveValue * OpenNijiiroRW.Skin.Game_Judge_Move[1]) + OpenNijiiroRW.stageGameScreen.GetJPOSCROLLY(j);

						OpenNijiiroRW.Tx.Judge.Opacity = (int)(255f - (judgeC.counter.CurrentValue >= 360 ? ((judgeC.counter.CurrentValue - 360) / 50.0f) * 255f : 0f));
						OpenNijiiroRW.Tx.Judge.t2D描画(x, y, judgeC.rc);
					}
				}
			}
		}
	...
	public void Start(int player, ENoteJudge judge)
	{
		...
		int height = OpenNijiiroRW.Tx.Judge.szTextureSize.Height / 5;
		judgeAnime.rc = new Rectangle(0, (int)njudge * height, OpenNijiiroRW.Tx.Judge.szTextureSize.Width, height);
