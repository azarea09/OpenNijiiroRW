...
internal class Dan_Cert : CActivity
{
	...
	public void Start(int number)
	{
		...
		if (number == 0)
		{
			Counter_Wait = new CCounter(0, 2299, 1, OpenNijiiroRW.Timer);
		}
		else
		{
			Counter_In = new CCounter(0, 999, 1, OpenNijiiroRW.Timer);
		}
		...
		if (number == 0)
		{
			...
			{
				if (OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[0].Dan_C[j] != null)
				{
					Challenge[j] = OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[NowShowingNumber].Dan_C[j];
					if (OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count - 1].Dan_C[j] != null
						&& OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count > 1) // Individual exams, not counted if dan is only a single song
					{
						OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[NowShowingNumber].Dan_C[j].Amount = 0;
						ExamChange[j] = true;
					}
				}
			}
		...
		OpenNijiiroRW.stageGameScreen.ReSetScore(OpenNijiiroRW.TJA.List_DanSongs[NowShowingNumber].ScoreInit, OpenNijiiroRW.TJA.List_DanSongs[NowShowingNumber].ScoreDiff, 0);

		OpenNijiiroRW.stageGameScreen.ftDanReSetScoreGen4ShinUchi(NowShowingNumber);
		OpenNijiiroRW.stageGameScreen.ftDanReSetBranches(OpenNijiiroRW.TJA.bHasBranchDan[NowShowingNumber]);
		...
		OpenNijiiroRW.stageGameScreen.actPanel.SetPanelString(OpenNijiiroRW.TJA.List_DanSongs[NowShowingNumber].Title,
	...
	public override void Activate()
	{
		for (int i = 0; i < CExamInfo.cMaxExam; i++)
		{
			if (OpenNijiiroRW.TJA.Dan_C[i] != null) Challenge[i] = new Dan_C(OpenNijiiroRW.TJA.Dan_C[i]);

			for (int j = 0; j < OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count; j++)
			{
				if (OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[j].Dan_C[i] != null)
				{
					OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[j].Dan_C[i] = new Dan_C(OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[j].Dan_C[i]);
				}
			}
		}

		if (OpenNijiiroRW.stageGameScreen.ListDan_Number >= 1 && FirstSectionAnime)
			OpenNijiiroRW.stageGameScreen.ListDan_Number = 0;

		FirstSectionAnime = false;
		// 始点を決定する。
		// ExamCount = 0;

		songsnotesremain = new int[OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count];
		this.ct虹アニメ = new CCounter(0, OpenNijiiroRW.Skin.Game_Gauge_Dan_Rainbow_Ptn - 1, 30, OpenNijiiroRW.Timer);
		this.ct虹透明度 = new CCounter(0, OpenNijiiroRW.Skin.Game_Gauge_Rainbow_Timer - 1, 1, OpenNijiiroRW.Timer);

		this.pfExamFont = HPrivateFastFont.tInstantiateMainFont(OpenNijiiroRW.Skin.Game_DanC_ExamFont_Size);
		...
		IsEnded = new bool[OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count];

		if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Dan) IsAnimating = true;
	...
	public void Update(bool resetFlash = false)
	{
		...
		for (int i = 0; i < CExamInfo.cMaxExam; i++)
		{
			...
			if (ExamChange[i] && Challenge[i] != OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[NowShowingNumber].Dan_C[i]) continue;
			...
			{
				Exam.Type.Gauge => OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値[0],
				...
				Exam.Type.Score => OpenNijiiroRW.stageGameScreen.actScore.GetScore(0),
			...
			if (isChangedAmount)
			{
				...
				{
					Status[i].Timer_Amount = new CCounter(0, 11, 12, OpenNijiiroRW.Timer);
			...
			if (Challenge[i].ReachStatus != oldReachedStatus && oldReachedStatus != Exam.ReachStatus.Unknown)
			{
				...
				this.Status[i].Timer_Gauge.Start(0, this.Status[i].Timer_Gauge.EndValue, 1, OpenNijiiroRW.Timer);
				SetFlashSpeed(this.Status[i], this.Challenge[i].ReachStatus);
			}
			else if (resetFlash && (this.NowShowingNumber == 0 || this.ExamChange[i]))
			{ // reset flash when changing gauge
				SetFlashSpeed(this.Status[i], this.Challenge[i].ReachStatus);
			}
		}
	}
	...
	private DanExamScore GetIndividualExamScore(int iSong)
	{
		...
		{
			judges = OpenNijiiroRW.stageGameScreen.DanSongScore[iSong],
			nCombo = OpenNijiiroRW.stageGameScreen.DanSongScore[iSong].nCombo,
			nHighestCombo = OpenNijiiroRW.stageGameScreen.DanSongScore[iSong].nHighestCombo,
			msBarRollMax = OpenNijiiroRW.stageGameScreen.nRollTimeMs_Dan[iSong],

			nNotesMax = OpenNijiiroRW.TJA!.nDan_NotesCount[iSong],
			nBarRollMax = OpenNijiiroRW.TJA.nDan_BarRollCount[iSong],
			nBalloonHitMax = OpenNijiiroRW.TJA.nDan_BalloonHitCount[iSong],
			nAdLibMax = OpenNijiiroRW.TJA.nDan_AdLibCount[iSong],
			nMineMax = OpenNijiiroRW.TJA.nDan_MineCount[iSong],

			lastChip = OpenNijiiroRW.TJA.pDan_LastChip[iSong],
			hasBranch = OpenNijiiroRW.TJA.bHasBranchDan[iSong],
		};
	...
	private DanExamScore GetTotalExamScore()
	{
		...
		{
			judges = OpenNijiiroRW.stageGameScreen.CChartScore[0],
			nCombo = OpenNijiiroRW.stageGameScreen.actCombo.nCurrentCombo.P1,
			nHighestCombo = OpenNijiiroRW.stageGameScreen.actCombo.nCurrentCombo.最高値[0],
			msBarRollMax = OpenNijiiroRW.stageGameScreen.nRollTimeMs_Dan.Sum(),

			nNotesMax = OpenNijiiroRW.TJA!.nノーツ数[3],
			nBarRollMax = OpenNijiiroRW.TJA.nDan_BarRollCount.Sum(),
			nBalloonHitMax = OpenNijiiroRW.TJA.nDan_BalloonHitCount.Sum(),
			nAdLibMax = OpenNijiiroRW.TJA.nDan_AdLibCount.Sum(),
			nMineMax = OpenNijiiroRW.TJA.nDan_MineCount.Sum(),

			lastChip = !(OpenNijiiroRW.TJA.pDan_LastChip.Length > 0) ? null : OpenNijiiroRW.TJA.pDan_LastChip[OpenNijiiroRW.TJA.pDan_LastChip.Length - 1],
			hasBranch = OpenNijiiroRW.TJA.bチップがある.Branch,
		};
	...
	private static void SetFlashSpeed(ChallengeStatus challengeStatus, Exam.ReachStatus reachStatus)
	{
		...
		if (flashSpeed > 0)
		{
			...
			challengeStatus.ctGaugeFlashLoop.Start(-msDelay, challengeStatus.ctGaugeFlashLoop.EndValue, 1.0 / flashSpeed, OpenNijiiroRW.Timer);
		}
		else
		{
	...
	private void UpdateReachStatus(int iSong, int iExam, DanExamScore score, bool isIndividualExam)
	{
		...
		bool isAfterLastChip = (lastChip == null)
			|| ((NotesManager.IsHittableNote(lastChip) && lastChip.bVisible) ?
				...
				: lastChip.n発声時刻ms <= OpenNijiiroRW.TJA.GameTimeToTjaTime(SoundManager.PlayTimer.NowTimeMs)
			);
		...
		{
			double msRemainBarRoll = CTja.TjaDurationToGameDuration(score.msBarRollMax - (score.judges!.msBarRollPass + OpenNijiiroRW.stageGameScreen.msCurrentBarRollProgress[0]));
		...
		switch (dan_C.ExamType)
		{
			...
			case Exam.Type.Score:
				...
				if (dan_C.ExamRange != Exam.Range.Less)
				{
					...
					for (int i = iSong; i < (isIndividualExam ? iSong + 1 : OpenNijiiroRW.stageGameScreen.nAddScoreGen4ShinUchi_Dan.Length); ++i)
						ptsMaxRemainingNote += OpenNijiiroRW.stageGameScreen.nAddScoreGen4ShinUchi_Dan[i] * this.GetIndividualExamScore(i).nNotesRemainMax;

					if (dan_C.GetExamStatus() < Exam.Status.Success)
					{
						int balloonHitRemainMax = score.nBalloonHitMax - score.judges!.nBalloonHitPass;
						double ptsMaxNoBarRoll = dan_C.Amount + ptsMaxRemainingNote + 100 * balloonHitRemainMax;
						if (judgeFailure && OpenNijiiroRW.ConfigIni.ShinuchiMode && (ptsMaxNoBarRoll < dan_C.GetValue()[0])
							&& (score.nBarRollMax <= score.judges!.nBarRollPass)
							)
						{
							dan_C.ReachStatus = Exam.ReachStatus.Failure;
				...
				{
					double ptsPerHitMax = OpenNijiiroRW.stageGameScreen.nAddScoreGen4ShinUchi_Dan.Skip(iSong).Append(100).Max();
	...
	public override void DeActivate()
	{
		...
		OpenNijiiroRW.tDisposeSafely(ref this.pfExamFont);
	...
	{
		Sound_Section = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Section.ogg"), ESoundGroup.SoundEffect);
		Sound_Section_First = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Section_First.wav"), ESoundGroup.SoundEffect);
		Sound_Failed = OpenNijiiroRW.SoundManager.tCreateSound(CSkin.Path(@$"Sounds{Path.DirectorySeparatorChar}Dan{Path.DirectorySeparatorChar}Failed.ogg"), ESoundGroup.SoundEffect);
	...
	{
		if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] != (int)Difficulty.Dan) return base.Draw();
		Counter_In?.Tick();
		Counter_Wait?.Tick();
		Counter_Out?.Tick();
		Counter_Text?.Tick();
		...
		OpenNijiiroRW.Tx.DanC_Background?.t2D描画(0, 0);

		DrawExam(this.Challenge, OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs);
		...
		if (Counter_In != null)
		{
			if (Counter_In.IsUnEnded)
			{
				...
				OpenNijiiroRW.Tx.DanC_Screen?.t2D描画((int)Math.Ceiling(ScreenPoint[0]) - OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Width / 2, OpenNijiiroRW.Skin.Game_Lane_Y[0], new Rectangle(0, 0, OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Width / 2, OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Height));
				OpenNijiiroRW.Tx.DanC_Screen?.t2D描画((int)Math.Floor(ScreenPoint[1]), OpenNijiiroRW.Skin.Game_Lane_Y[0], new Rectangle(OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Width / 2, 0, OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Width / 2, OpenNijiiroRW.Tx.DanC_Screen.szTextureSize.Height));
		...
		if (Counter_Wait != null)
		{
			if (Counter_Wait.IsUnEnded)
			{
				...
				Counter_In = null;
				OpenNijiiroRW.Tx.DanC_Screen?.t2D描画(OpenNijiiroRW.Skin.Game_Lane_X[0], OpenNijiiroRW.Skin.Game_Lane_Y[0]);
					...
					if (Counter_Wait.CurrentValue >= 800)
					{
						if (!bExamChangeCheck)
						{
							...
							for (int j = 0; j < CExamInfo.cMaxExam; j++)
							{
								if (OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[0].Dan_C[j] != null)
								{
									if (OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs.Count - 1].Dan_C[j] != null) //個別の条件がありますよー
									{
										Challenge[j] = OpenNijiiroRW.stageSongSelect.rChoosenSong.DanSongs[NowShowingNumber].Dan_C[j];
										ExamChange[j] = true;
									}
			...
			{
				Counter_Wait = null;
				Counter_Out = new CCounter(0, 90, 3, OpenNijiiroRW.Timer);
				Counter_Text = new CCounter(0, 2899, 1, OpenNijiiroRW.Timer);
			}
		}
		if (Counter_Text != null)
		{
			...
			{
				if (OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].TitleTex != null)
				{
					OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].TitleTex.Opacity = Math.Max(0, 255 - (Counter_Text.CurrentValue - 2000) / 2);
				}
				if (OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].SubTitleTex != null)
				{
					OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].SubTitleTex.Opacity = Math.Max(0, 255 - (Counter_Text.CurrentValue - 2000) / 2);
				}
			}
			else
			{
				if (OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].TitleTex != null)
				{
					OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].TitleTex.Opacity = 255;
				}
				if (OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].SubTitleTex != null)
				{
					OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].SubTitleTex.Opacity = 255;
				}
			}
			if (Counter_Text.IsUnEnded)
			{

				var title = OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].TitleTex;
				var subTitle = OpenNijiiroRW.TJA.List_DanSongs[NowCymbolShowingNumber].SubTitleTex;
				if (subTitle == null)
					title?.t2D拡大率考慮中央基準描画(OpenNijiiroRW.Skin.Game_DanC_Title_X[0], OpenNijiiroRW.Skin.Game_DanC_Title_Y[0]);
				else
				{
					title?.t2D拡大率考慮中央基準描画(OpenNijiiroRW.Skin.Game_DanC_Title_X[1], OpenNijiiroRW.Skin.Game_DanC_Title_Y[1]);
					subTitle?.t2D拡大率考慮中央基準描画(OpenNijiiroRW.Skin.Game_DanC_SubTitle[0], OpenNijiiroRW.Skin.Game_DanC_SubTitle[1]);
				}
			}
		...
		CActSelect段位リスト.tDisplayDanPlate(Dan_Plate,
			null,
			OpenNijiiroRW.Skin.Game_DanC_Dan_Plate[0],
			OpenNijiiroRW.Skin.Game_DanC_Dan_Plate[1]);
	...
	public void DrawExam(ReadOnlySpan<Dan_C> dan_C, List<CTja.DanSongs> danSongs, bool isResult = false, int offX = 0)
	{
		...
		for (int i = 0; i < CExamInfo.cMaxExam; i++)
		{
				...
				|| isResult)
			{
				...
				// Panel origin
				int xOrigin = (isResult) ? OpenNijiiroRW.Skin.DanResult_Exam[0] + offX : OpenNijiiroRW.Skin.Game_DanC_X[1];
				int yOrigin = (isResult) ? OpenNijiiroRW.Skin.DanResult_Exam[1] : OpenNijiiroRW.Skin.Game_DanC_Y[1];

				// Origin position which will be used as a reference for bar elements
				int barXOffset = xOrigin + (currentPosition >= 3 ? OpenNijiiroRW.Skin.Game_DanC_Base_Offset_X[1] : OpenNijiiroRW.Skin.Game_DanC_Base_Offset_X[0]);
				int barYOffset = yOrigin + (currentPosition >= 3 ? OpenNijiiroRW.Skin.Game_DanC_Base_Offset_Y[1] : OpenNijiiroRW.Skin.Game_DanC_Base_Offset_Y[0]) + OpenNijiiroRW.Skin.Game_DanC_Size[1] * yIndex + (yIndex * OpenNijiiroRW.Skin.Game_DanC_Padding);

				// Small bar
				int lowerBarYOffset = barYOffset + OpenNijiiroRW.Skin.Game_DanC_Size[1] + OpenNijiiroRW.Skin.Game_DanC_Padding;
				...
				if (!isSmallGauge)
					OpenNijiiroRW.Tx.DanC_Base?.t2D描画(barXOffset, barYOffset, new RectangleF(0, ExamChange[i] ? OpenNijiiroRW.Tx.DanC_Base.szTextureSize.Height / 2 : 0, OpenNijiiroRW.Tx.DanC_Base.szTextureSize.Width, OpenNijiiroRW.Tx.DanC_Base.szTextureSize.Height / 2));
				else
					OpenNijiiroRW.Tx.DanC_Base_Small?.t2D描画(barXOffset, barYOffset, new RectangleF(0, ExamChange[i] ? OpenNijiiroRW.Tx.DanC_Base_Small.szTextureSize.Height / 2 : 0, OpenNijiiroRW.Tx.DanC_Base_Small.szTextureSize.Width, OpenNijiiroRW.Tx.DanC_Base_Small.szTextureSize.Height / 2));
				...
				if (ExamChange[i] == true)
				{
					for (int j = 0; j < danSongs.Count - 1; j++)
					{
						...
						OpenNijiiroRW.Tx.DanC_SmallBase.Opacity = opacityJ;
						OpenNijiiroRW.Tx.DanC_Small_ExamCymbol.Opacity = opacityJ;

						OpenNijiiroRW.Tx.Gauge_Dan_Rainbow[0].Opacity = opacityJ;
						OpenNijiiroRW.Tx.DanC_MiniNumber.Opacity = opacityJ;

						OpenNijiiroRW.Tx.DanC_Gauge[idxExamGaugeTextureJ].Opacity = opacityJ;
						...
						int miniBarPositionX = barXOffset + (isSmallGauge ? OpenNijiiroRW.Skin.Game_DanC_SmallBase_Offset_X[1] : OpenNijiiroRW.Skin.Game_DanC_SmallBase_Offset_X[0]);

						// 613 + j * 33 : Small base (barYoffset for 3rd exam : 494 + 119 + Local song offset j * 33)
						int miniBarPositionY = (barYOffset + (isSmallGauge ? OpenNijiiroRW.Skin.Game_DanC_SmallBase_Offset_Y[1] : OpenNijiiroRW.Skin.Game_DanC_SmallBase_Offset_Y[0])) + (j % 2) * smallBarGap - (OpenNijiiroRW.Skin.Game_DanC_Size[1] + (OpenNijiiroRW.Skin.Game_DanC_Padding));

						// Display bars
						#region [Displayables]

						// Display mini-bar base and small symbol
						OpenNijiiroRW.Tx.DanC_SmallBase?.t2D描画(miniBarPositionX, miniBarPositionY);
						OpenNijiiroRW.Tx.DanC_Small_ExamCymbol?.t2D描画(miniBarPositionX - 30, miniBarPositionY - 3, new RectangleF(0, j * 28, 30, 28));
						...
						int _tmpMiniPadding = (int)(14f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
						...
						CActSelect段位リスト.tDisplayDanIcon(j + 1, miniBarPositionX + OpenNijiiroRW.Skin.Game_DanC_DanIcon_Offset_Mini[0], miniBarPositionY + OpenNijiiroRW.Skin.Game_DanC_DanIcon_Offset_Mini[1], miniIconOpacity, 0.5f, false);
				...
				OpenNijiiroRW.Tx.DanC_ExamCymbol.Opacity = 255;
				...
				if (ExamChange[i] && NowShowingNumber != 0)
				{
					...
					{
						if (Counter_Wait.CurrentValue >= 800)
							OpenNijiiroRW.Tx.DanC_ExamCymbol.Opacity = examChangeFadeInOpacity;
						else if (Counter_Wait.CurrentValue >= 800 - 255)
							OpenNijiiroRW.Tx.DanC_ExamCymbol.Opacity = examChangeFadeOutOpacity;
					}
				}
				...
				if (ExamChange[i])
				{
					OpenNijiiroRW.Tx.DanC_ExamCymbol.t2D描画(barXOffset + 5, lowerBarYOffset - 64, new RectangleF(0, 41 * NowCymbolShowingNumber, 197, 41));
				}
				...
				OpenNijiiroRW.Tx.DanC_Gauge[idxExamGaugeTexture].Opacity = gaugeOpacity;

				OpenNijiiroRW.Tx.Gauge_Dan_Rainbow[rainbowIndex].Opacity = gaugeOpacity;

				OpenNijiiroRW.Tx.DanC_Number.Opacity = opacity;
				OpenNijiiroRW.Tx.DanC_ExamRange.Opacity = opacity;
				OpenNijiiroRW.Tx.DanC_Small_Number.Opacity = opacity;
				...
				if (dan_C[i].ReachStatus == Exam.ReachStatus.Better_Success)
				{
					...
					var gaugeTexture = OpenNijiiroRW.Tx.Gauge_Dan_Rainbow[rainbowIndex];
					...
					gaugeTexture = OpenNijiiroRW.Tx.Gauge_Dan_Rainbow[rainbowBase];
					if (Counter_Wait != null && !(Counter_Wait.CurrentValue <= 1055 && Counter_Wait.CurrentValue >= 800 - 255))
					{
				...
				else
				{
					...
					drawGauge(OpenNijiiroRW.Tx.DanC_Gauge[idxExamGaugeTexture], dan_C);
				}
				...
				float numberXScale = isSmallGauge ? OpenNijiiroRW.Skin.Game_DanC_Number_Small_Scale * 0.6f : OpenNijiiroRW.Skin.Game_DanC_Number_Small_Scale;
				float numberYScale = isSmallGauge ? OpenNijiiroRW.Skin.Game_DanC_Number_Small_Scale * 0.8f : OpenNijiiroRW.Skin.Game_DanC_Number_Small_Scale;
				int numberPadding = (int)(OpenNijiiroRW.Skin.Game_DanC_Number_Padding * (isSmallGauge ? 0.6f : 1f));

				DrawNumber(dan_C[i].GetDisplayedAmount(),
					barXOffset + OpenNijiiroRW.Skin.Game_DanC_Number_Small_Number_Offset[0],
					lowerBarYOffset - OpenNijiiroRW.Skin.Game_DanC_Number_Small_Number_Offset[1],
					numberPadding,
					true,
					dan_C[i].ReachStatus,
					numberXScale,
				...
				if (ExamChange[i])
				{
					CActSelect段位リスト.tDisplayDanIcon(this.NowCymbolShowingNumber + 1, barXOffset + OpenNijiiroRW.Skin.Game_DanC_DanIcon_Offset[0], barYOffset + OpenNijiiroRW.Skin.Game_DanC_DanIcon_Offset[1], iconOpacity, 0.6f, true);
				}
				...
				OpenNijiiroRW.Tx.DanC_ExamRange?.t2D拡大率考慮下基準描画(
					barXOffset + offset - OpenNijiiroRW.Tx.DanC_ExamRange.szTextureSize.Width,
					lowerBarYOffset - OpenNijiiroRW.Skin.Game_DanC_Exam_Offset[1],
					new Rectangle(0, OpenNijiiroRW.Skin.Game_DanC_ExamRange_Size[1] * (int)dan_C[i].ExamRange, OpenNijiiroRW.Skin.Game_DanC_ExamRange_Size[0], OpenNijiiroRW.Skin.Game_DanC_ExamRange_Size[1]));

				offset -= OpenNijiiroRW.Skin.Game_DanC_ExamRange_Padding;

				// Condition number
				DrawNumber(
					dan_C[i].GetValue()[0],
					barXOffset + offset - dan_C[i].GetValue()[0].ToString().Length * (int)(OpenNijiiroRW.Skin.Game_DanC_Number_Small_Padding * OpenNijiiroRW.Skin.Game_DanC_Exam_Number_Scale),
					lowerBarYOffset - OpenNijiiroRW.Skin.Game_DanC_Exam_Offset[1] - 1,
					(int)(OpenNijiiroRW.Skin.Game_DanC_Number_Small_Padding * OpenNijiiroRW.Skin.Game_DanC_Exam_Number_Scale),
					false,
					dan_C[i].ReachStatus);

				int _offexX = (int)(22f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
				int _offexY = (int)(48f * OpenNijiiroRW.Skin.Resolution[1] / 720f);
				int _examX = barXOffset + OpenNijiiroRW.Skin.Game_DanC_Exam_Offset[0] - OpenNijiiroRW.Tx.DanC_ExamType.szTextureSize.Width + _offexX;
				int _examY = lowerBarYOffset - OpenNijiiroRW.Skin.Game_DanC_Exam_Offset[1] - _offexY;

				// Exam type flag
				OpenNijiiroRW.Tx.DanC_ExamType?.t2D拡大率考慮下基準描画(
					_examX,
					_examY,
					new Rectangle(0, 0, OpenNijiiroRW.Skin.Game_DanC_ExamType_Size[0], OpenNijiiroRW.Skin.Game_DanC_ExamType_Size[1]));
				...
				if ((int)dan_C[i].ExamType < this.ttkExams.Length)
					TitleTextureKey.ResolveTitleTexture(this.ttkExams[(int)dan_C[i].ExamType]).t2D拡大率考慮中央基準描画(
						_examX + OpenNijiiroRW.Skin.Game_DanC_ExamType_Size[0] / 2,
						_examY - OpenNijiiroRW.Skin.Game_DanC_ExamType_Size[1] / 2);
                ...
                */
				...
				if (dan_C[i].ReachStatus == Exam.ReachStatus.Failure)
				{
					...
					OpenNijiiroRW.Tx.DanC_Failed.t2D拡大率考慮下基準描画(
						barXOffset + OpenNijiiroRW.Skin.Game_DanC_Offset[0],
						lowerBarYOffset - OpenNijiiroRW.Skin.Game_DanC_Offset[1]);
				}
			...
			else
			{
				...
				int _scale = (int)(14f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
				int _nbX = (int)(292f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
				int _nbY = (int)(64f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
				int _offexX = (int)(104f * OpenNijiiroRW.Skin.Resolution[0] / 1280f);
				int _offexY = (int)(21f * OpenNijiiroRW.Skin.Resolution[1] / 720f);

				OpenNijiiroRW.Tx.DanC_Gauge_Base?.t2D描画(
					OpenNijiiroRW.Skin.Game_DanC_X[0] - ((50 - dan_C[i].GetValue()[0] / 2) * _scale) + 4,
					OpenNijiiroRW.Skin.Game_DanC_Y[0]);

				TitleTextureKey.ResolveTitleTexture(this.ttkExams[(int)Exam.Type.Gauge]).t2D拡大率考慮中央基準描画(
					OpenNijiiroRW.Skin.Game_DanC_X[0] - ((50 - dan_C[i].GetValue()[0] / 2) * _scale) + _offexX,
					OpenNijiiroRW.Skin.Game_DanC_Y[0] + _offexY);

				// Display percentage here
				DrawNumber(
					dan_C[i].GetValue()[0],
					OpenNijiiroRW.Skin.Game_DanC_X[0] - ((50 - dan_C[i].GetValue()[0] / 2) * _scale) + _nbX - dan_C[i].GetValue()[0].ToString().Length * (int)(OpenNijiiroRW.Skin.Game_DanC_Number_Small_Padding * OpenNijiiroRW.Skin.Game_DanC_Exam_Number_Scale),
					OpenNijiiroRW.Skin.Game_DanC_Y[0] - OpenNijiiroRW.Skin.Game_DanC_Exam_Offset[1] + _nbY,
					(int)(OpenNijiiroRW.Skin.Game_DanC_Number_Small_Padding * OpenNijiiroRW.Skin.Game_DanC_Exam_Number_Scale),
					false,
					dan_C[i].ReachStatus);
	...
	{
		if (OpenNijiiroRW.Tx.DanC_Number == null || OpenNijiiroRW.Tx.DanC_Small_Number == null || value < 0)
			return;

		if (value == 0 || reachStatus == Exam.ReachStatus.Failure)
		{
			OpenNijiiroRW.Tx.DanC_Number.color4 = CConversion.ColorToColor4(Color.Gray);
			OpenNijiiroRW.Tx.DanC_Small_Number.color4 = CConversion.ColorToColor4(Color.Gray);
		}
		else
		{
			OpenNijiiroRW.Tx.DanC_Number.color4 = CConversion.ColorToColor4(Color.White);
			OpenNijiiroRW.Tx.DanC_Small_Number.color4 = CConversion.ColorToColor4(Color.White);
		}
	...
	{
		if (OpenNijiiroRW.Tx.DanC_MiniNumber == null || value < 0)
			return;
		...
		{
			var number = Convert.ToInt32(value.ToString()[i].ToString());
			Rectangle rectangle = new Rectangle(OpenNijiiroRW.Skin.Game_DanC_MiniNumber_Size[0] * number - 1, (reachStatus == Exam.ReachStatus.Better_Success) ? OpenNijiiroRW.Skin.Game_DanC_MiniNumber_Size[1] : 0, OpenNijiiroRW.Skin.Game_DanC_MiniNumber_Size[0], OpenNijiiroRW.Skin.Game_DanC_MiniNumber_Size[1]);
			OpenNijiiroRW.Tx.DanC_MiniNumber.t2D拡大率考慮下中心基準描画(x - (notesRemainDigit * padding), y, rectangle);
			notesRemainDigit--;
		}
	}
	...
	public Exam.Status GetResultExamStatus(ReadOnlySpan<Dan_C> dan_C, List<CTja.DanSongs> danSongs)
	{
		...
		for (int i = 0; i < CExamInfo.cMaxExam; i++)
		{
			...
			{
				for (int j = 0; j < danSongs.Count - 1; j++)
				{ // last song already being checked
	...
	{
		0 => OpenNijiiroRW.Skin.Game_Lane_X[0],
		1 => OpenNijiiroRW.Skin.Game_Lane_X[0] + (OpenNijiiroRW.Skin.Resolution[0] - OpenNijiiroRW.Skin.Game_Lane_X[0]) / 2.0,
		_ => OpenNijiiroRW.Skin.Resolution[0],
	};
