...
class ScriptBG : IDisposable {
	...
	public void Init() {
		...
		try {
			...
			// Initialisation
			LuaSetConstValues.Call(OpenNijiiroRW.ConfigIni.nPlayerCount,
				OpenNijiiroRW.P1IsBlue(),
				OpenNijiiroRW.ConfigIni.sLang,
				OpenNijiiroRW.ConfigIni.SimpleMode,
				raritiesP,
				raritiesC
			);

			LuaUpdateValues.Call(OpenNijiiroRW.FPS.DeltaTime,
				OpenNijiiroRW.FPS.NowFPS,
				OpenNijiiroRW.stageGameScreen.bIsAlreadyCleared,
				0,
				OpenNijiiroRW.stageGameScreen.AIBattleState,
				OpenNijiiroRW.stageGameScreen.bIsAIBattleWin,
				OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値,
				OpenNijiiroRW.stageGameScreen.actPlayInfo.dbBPM,
	...
	public void Update() {
		...
		try {
			...
			if (OpenNijiiroRW.TJA != null) {
				...
				timestamp = (OpenNijiiroRW.TJA.RawTjaTimeToDefTime(
					OpenNijiiroRW.TJA.TjaTimeToRawTjaTimeNote(
						OpenNijiiroRW.TJA.GameTimeToTjaTime(SoundManager.PlayTimer.NowTimeMs))
				) + msTimeOffset) / 1000.0;
			}
			...
			LuaUpdateValues.Call(OpenNijiiroRW.FPS.DeltaTime,
				...
				OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値,
				OpenNijiiroRW.stageGameScreen.actPlayInfo.dbBPM,
				OpenNijiiroRW.stageGameScreen.bIsGOGOTIME,
				timestamp);
