...
internal class CActImplGauge : CAct演奏ゲージ共通 {
	...
	public override void Activate() {
		...
		this.ctGaugeFlash = new CCounter(0, 532, 1, OpenNijiiroRW.Timer);
	...
	public override int Draw() {
		if (!base.IsDeActivated) {
			...
			int nWidth = (OpenNijiiroRW.Skin.Game_Gauge_Rect[2] / 50);
			int[] nRectX = new int[] {
            ...
            */
			...
			// No gauge if tower
			if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Tower || OpenNijiiroRW.ConfigIni.bTokkunMode)
				return 0;
				...
				if (OpenNijiiroRW.Tx.Gauge[0] != null) {
					...
					if (OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Dan) {
						...
						for (int i = 0; i < OpenNijiiroRW.TJA.Dan_C.Length; i++) {
							...
							if (dan_c != null && dan_c.ExamType == Exam.Type.Gauge && db現在のゲージ値[0] >= dan_c.GetValue()[0]) {
								...
								OpenNijiiroRW.Tx.Gauge_Dan[3].Opacity = Opacity;
								OpenNijiiroRW.Tx.Gauge_Dan[3]?.t2D描画(x, y, new Rectangle(0, 0, dan_c.GetValue()[0] / 2 * nWidth, OpenNijiiroRW.Skin.Game_Gauge_Rect[3]));
					...
					if (this.db現在のゲージ値[0] >= 100.0) {
						...
						if (OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue] != null) {
							...
							OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue].Opacity = 255;
							OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue].t2D描画(x, y + (smart ? (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : 0),
								new RectangleF(0,
									smart ? (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : 0,
									OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue].szTextureSize.Width,
									smart ? OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue].szTextureSize.Height - (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : OpenNijiiroRW.Tx.Gauge_Rainbow[this.ct虹アニメ.CurrentValue].szTextureSize.Height));


							OpenNijiiroRW.Tx.Gauge_Rainbow[虹ベース].Opacity = (ct虹透明度.CurrentValue * 255 / (int)ct虹透明度.EndValue) / 1;
							OpenNijiiroRW.Tx.Gauge_Rainbow[虹ベース].t2D描画(x, y + (smart ? (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : 0),
								new RectangleF(0,
									smart ? (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : 0,
									OpenNijiiroRW.Tx.Gauge_Rainbow[虹ベース].szTextureSize.Width,
									smart ? OpenNijiiroRW.Tx.Gauge_Rainbow[虹ベース].szTextureSize.Height - (OpenNijiiroRW.Skin.Game_Gauge_Rect[3] / 2) : OpenNijiiroRW.Tx.Gauge_Rainbow[虹ベース].szTextureSize.Height));
						}
					}
