		...
		if (OpenNijiiroRW.Tx.Notes[(int)_gt] != null)
		{
			for (int i = 0; i < 128; i++)
			{
				if (!Flying[i].IsUsing)
				{
					...
					Flying[i].StartPointY = OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_StartPoint_Y[nPlayer];
					Flying[i].OldValue = 0;
					Flying[i].IsRoll = isRoll;
					// 角度の決定
					Flying[i].Height = Math.Abs(OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_EndPoint_Y[nPlayer] - OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_StartPoint_Y[nPlayer]);
					Flying[i].Width = (Math.Abs((OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_EndPoint_X[nPlayer] - StartPointX[nPlayer])) / 2);
					//Console.WriteLine("{0}, {1}", width2P, height2P);
					Flying[i].Theta = ((Math.Atan2(Flying[i].Height, Flying[i].Width) * 180.0) / Math.PI);
					Flying[i].Counter = new CCounter(0, 140, OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_Timer, OpenNijiiroRW.Timer);
					//Flying[i].Counter = new CCounter(0, 200000, CDTXMania.Skin.Game_Effect_FlyingNotes_Timer, CDTXMania.Timer);

					Flying[i].IncreaseX = (1.00 * Math.Abs((OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_EndPoint_X[nPlayer] - StartPointX[nPlayer]))) / (180);
					Flying[i].IncreaseY = (1.00 * Math.Abs((OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_EndPoint_Y[nPlayer] - OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_StartPoint_Y[nPlayer]))) / (180);
					break;
				}
			}
		}
			...
			for (int i = 0; i < 128; i++)
			{
				if (Flying[i].IsUsing)
				{
					...
					for (int n = Flying[i].OldValue; n < Flying[i].Counter.CurrentValue; n += 16)
					{
						...
						int movingDistanceY = endY - OpenNijiiroRW.Skin.Game_Effect_FlyingNotes_StartPoint_Y[Flying[i].Player];
