...
class CSongReplay {
	...
	public void tDanInputSongResults(int songNo) {
		...
		IndividualOkCount[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nGood;
		IndividualBadCount[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nMiss;
		IndividualRollCount[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nRoll;
		IndividualMaxCombo[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nHighestCombo;
		IndividualBoomCount[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nMine;
		IndividualADLibCount[songNo] = OpenNijiiroRW.stageGameScreen.DanSongScore[songNo].nADLIB;
		danAccumulatedScore = 0;
		for (int acc = 0; acc < songNo; acc++) danAccumulatedScore += IndividualScore[acc];
		IndividualScore[songNo] = (int)OpenNijiiroRW.stageGameScreen.actScore.GetScore(0) - danAccumulatedScore;
	}
	...
	public void tResultsRegisterReplayInformations(int Coins, int Clear, int SRank) {
		...
		ChartChecksum = "";
		// Player Name
		PlayerName = OpenNijiiroRW.SaveFileInstances[actualPlayer].data.Name;
		// Performance informations
		GoodCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nGreat;
		OkCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nGood;
		BadCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nMiss;
		RollCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nRoll;
		MaxCombo = OpenNijiiroRW.stageGameScreen.actCombo.nCurrentCombo.最高値[storedPlayer];
		BoomCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nMine;
		ADLibCount = OpenNijiiroRW.stageGameScreen.CChartScore[storedPlayer].nADLIB;
		...
		ScrollSpeedValue = OpenNijiiroRW.ConfigIni.nScrollSpeed[actualPlayer];
		// Song speed value (as on ConfigIni, 20 is x1)
		SongSpeedValue = OpenNijiiroRW.ConfigIni.nSongSpeed;
		// Just strictess adjust mod value (as on ConfigIni, between -2 for lenient and 2 for rigorous)
		JudgeStrictnessAdjust = OpenNijiiroRW.ConfigIni.nTimingZones[actualPlayer];
		...
		if (OpenNijiiroRW.ConfigIni.eRandom[actualPlayer] == ERandomMode.Mirror) ModFlags |= (int)EModFlag.Mirror;
		if (OpenNijiiroRW.ConfigIni.eRandom[actualPlayer] == ERandomMode.Random) ModFlags |= (int)EModFlag.Random;
		if (OpenNijiiroRW.ConfigIni.eRandom[actualPlayer] == ERandomMode.SuperRandom) ModFlags |= (int)EModFlag.SuperRandom;
		if (OpenNijiiroRW.ConfigIni.eRandom[actualPlayer] == ERandomMode.MirrorRandom) ModFlags |= ((int)EModFlag.Random | (int)EModFlag.Mirror);
		if (OpenNijiiroRW.ConfigIni.eSTEALTH[actualPlayer] == EStealthMode.Doron) ModFlags |= (int)EModFlag.Invisible;
		if (OpenNijiiroRW.ConfigIni.eSTEALTH[actualPlayer] == EStealthMode.Stealth) ModFlags |= (int)EModFlag.PerfectMemory;
		if (OpenNijiiroRW.ConfigIni.nFunMods[actualPlayer] == EFunMods.Avalanche) ModFlags |= (int)EModFlag.Avalanche;
		if (OpenNijiiroRW.ConfigIni.nFunMods[actualPlayer] == EFunMods.Minesweeper) ModFlags |= (int)EModFlag.Minesweeper;
		if (OpenNijiiroRW.ConfigIni.bJust[actualPlayer] == 1) ModFlags |= (int)EModFlag.Just;
		if (OpenNijiiroRW.ConfigIni.bJust[actualPlayer] == 2) ModFlags |= (int)EModFlag.Safe;
		/* Gauge type
		 * - 0 : Normal
		 * - 1 : Hard
		 * - 2 : Extreme
		 */
		var chara = OpenNijiiroRW.Tx.Characters[OpenNijiiroRW.SaveFileInstances[actualPlayer].data.Character];
		GaugeType = (byte)HGaugeMethods.tGetGaugeTypeEnum(chara.effect.tGetGaugeType());
		// Gauge fill value
		GaugeFill = (float)OpenNijiiroRW.stageGameScreen.actGauge.db現在のゲージ値[storedPlayer];
		...
		// Chart metadata
		ChartUniqueID = OpenNijiiroRW.stageSongSelect.rChoosenSong.uniqueId.data.id;
		ChartDifficulty = (byte)OpenNijiiroRW.stageSongSelect.nChoosenSongDifficulty[storedPlayer];
		ChartLevel = (byte)Math.Min(255, OpenNijiiroRW.stageSongSelect.rChoosenSong.score[ChartDifficulty].譜面情報.nレベル[ChartDifficulty]);
