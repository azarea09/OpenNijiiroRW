...
public class CTexture : IDisposable
{
	...
	public void MakeTexture(string strファイル名, bool b黒を透過する)
	{
		...
		if (Path.GetExtension(strファイル名) == ".dds")
		{
			bitmap = LoadFromDDSToSKBitmap(strファイル名);
		}
		else
		{
			bitmap = SKBitmap.Decode(strファイル名);
		}
	...
	private SKBitmap LoadFromDDSToSKBitmap(string path)
	{
		SKBitmap sKBitmap;
